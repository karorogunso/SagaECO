    ...
    public class ProxyClient : Client
    {
        ...
        public void OnSendUniversal(Packets.Client.SendUniversal p)
        {
            try
            {
                ...
                frmMain.Instance.Invoke(new Action(() => { frmMain.Instance.PacketsList.Items.Add(string.Format("0x{0:X4},{1},Client,{2},{3}", p.ID, p.data.Length, this.packetContainer.IndexOf(p1), ms)); }));
