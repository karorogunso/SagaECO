...
namespace SagaScript.M30010003
{
    public class S11000435 : Event
    {
        ...
        void 埃米爾介紹書(ActorPC pc)
        {
            ...
            Say(pc, 11000435, 131, "那么，要挑战任务吗?$R;", "酒馆店员");
        ...
        void 任務種類詳細解說(ActorPC pc)
        {
            ...
            Say(pc, 11000435, 131, "任务的要求几乎都很简单喔!$R;" +
                                   "$R「咖啡馆」除了卖粮食外，$R;" +
                                   "也会介绍一些工作给冒险者唷!$R;" +
                                   "$P久而久之，口碑越来越好了，$R;" +
                                   "所以在「阿高普路斯市」周围，$R;" +
            ...
            while (selection != 5)
            {
                ...
                {
                    case 1:
                        Say(pc, 11000435, 131, "成功完成任务的话，$R;" +
                                               "可以得到相对应的经验值和报酬。$R;" +
                                               "$R但托付的任务并不是很多，$R;" +
                                               "有时候会供过于求，不太平衡啊!$R;" +
                                               "$P所以工作是有次数限制的。$R;" +
                    ...
                    case 2:
                        Say(pc, 11000435, 131, "「击退任务」就是要在指定的区域，$R;" +
                                               "抓到指定数量的魔物。$R;" +
                                               "$P例如：$R;" +
                                               "击退「阿克罗尼亚东方平原」的$R;" +
                                               "5隻「皮露露」。$R;" +
                    ...
                    case 3:
                        Say(pc, 11000435, 131, "「收集任务」就是收集指定道具的任务唷!$R;" +
                                               "$P如果接到收集3个『杰利科』的任务。$R;" +
                                               "$R只要想尽办法收集3个『杰利科』，$R;" +
                                               "就算任务完成了。$R;" +
                                               "$P收集完以后，$R;" +
                    ...
                    case 4:
                        Say(pc, 11000435, 131, "「搬运任务」是从委托人那裡取得道具，$R;" +
                                               "然后转交给收件人的任务哦!$R;" +
                                               "$P例如：$R;" +
                                               "在「阿克罗尼亚东方平原」的$R;" +
                                               "「酒馆店员」那裡$R;" +
        ...
        void 擊退皮露露(ActorPC pc)
        {
            ...
            Say(pc, 11000435, 131, "最近「阿克罗波利斯」周围，$R;" +
                                   "出现了很多「皮露露」，$R;" +
                                   "能不能击退呢?$R;" +
                                   "$R「皮露露」是像布丁的天蓝色魔物。$R;" +
                                   "$P在任务清单选择任务后，$R;" +
