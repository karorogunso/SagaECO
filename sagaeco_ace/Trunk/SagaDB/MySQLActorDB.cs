    ...
    public class MySQLActorDB : MySQLConnectivity, ActorDB
    {
        ...
        public void SaveChar(ActorPC aChar, bool itemInfo, bool fullinfo)
        {
            ...
            if (aChar != null)
            {
                ...
                sqlstr = string.Format("UPDATE `char` SET `race`='{1}',`gender`='{2}',`hairStyle`='{3}',`hairColor`='{4}',`wig`='{5}'," +
                    ...
                    "`dstr`='{73}',`ddex`='{74}',`dintel`='{75}',`dvit`='{76}',`dagi`='{77}',`dmag`='{78}',`dstatpoint`='{79}'," +
                    "`dreserve`='{80}',`ep`='{81}',`eplogindate`='{82}',`epgreetingdate`='{83}',`cl`='{84}',`dcl`='{85}'," +
                    "`epused`='{86}',`depused`='{87}',`tailStyle`='{88}',`wingStyle`='{89}',`wingColor`='{90}',`lv1`='{91}',`jlv3`='{92}',`explorerEXP`='{94}',`WaitType`='{95}'" +
                    " WHERE char_id='{15}' LIMIT 1",
                    ...
                    aChar.DominionMag, aChar.DominionStatsPoint, dreserve, aChar.EP, ToSQLDateTime(aChar.EPLoginTime), ToSQLDateTime(aChar.EPGreetingTime),
                    aChar.CL, aChar.DominionCL, aChar.EPUsed, aChar.DominionEPUsed, aChar.TailStyle, aChar.WingStyle, aChar.WingColor,
                    aChar.Level1, aChar.JobLevel3, aChar.SkillPoint3, aChar.ExplorerEXP, aChar.WaitType);
        ...
        public ActorPC GetChar(uint charID, bool fullinfo)
        {
            ...
            try
            {
                ...
                pc.DominionStr = ushort.Parse(result["dstr"].ToString());
                pc.DominionDex = ushort.Parse(result["ddex"].ToString());
                pc.DominionInt = ushort.Parse(result["dintel"].ToString());
                pc.DominionVit = ushort.Parse(result["dvit"].ToString());
                pc.DominionAgi = ushort.Parse(result["dagi"].ToString());
                pc.DominionMag = ushort.Parse(result["dmag"].ToString());
                pc.StatsPoint = ushort.Parse(result["statspoint"].ToString());
                pc.DominionStatsPoint = ushort.Parse(result["dstatpoint"].ToString());
                ...
                pc.DominionCEXP = ulong.Parse(result["dcexp"].ToString());
                pc.DominionJEXP = ulong.Parse(result["djexp"].ToString());
        ...
        public void SaveSkill(ActorPC pc)
        {
            ...
            //string sqlstr = string.Format("DELETE FROM `skill` WHERE `char_id`='{0}';", pc.CharID);
