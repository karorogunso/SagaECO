...
namespace SagaMap.Dungeon
{
    public class DungeonFactory : Factory<DungeonFactory, Dungeon>
    {
        ...
        public Dungeon GetDungeon(ActorPC pc)
        {
            return dungeons.FirstOrDefault(x => x.Value.Party == pc.Party).Value;
        }
        ...
        public int CreateDungeon(uint id, ActorPC pc, uint exitMap, byte exitX, byte exitY)
        {
            if (pc.Party == null)
                return -1;
            if (dungeons.Where(x => x.Value.Party == pc.Party).ToList().Count != 0)
                return -2;
            return (int)CreateDungeon(id, pc.Party.Leader, exitMap, exitX, exitY, true).DungeonID;
        }

        public Dungeon CreateDungeon(uint id, ActorPC creator, uint exitMap, byte exitX, byte exitY, bool creatorisPL)
        {
        recreate:
            if (items.ContainsKey(id))
            {
                ...
                dungeon.Start.Map = MapManager.Instance.GetMap(MapManager.Instance.CreateMapInstance(creator, dungeon.StartMap, exitMap, exitX, exitY, true));
                ...
                Dictionary<uint, List<SpawnData>> sd = Mob.MobSpawnManager.Instance.LoadDungeonSpawn(dungeon.SpawnFile);
                ...
                while (roomcount > 0 || floorcount > 0 || crosscount > 0 || endcount > 0)
                {
                    ...
                    if (ifEnd)
                        ...
                        Mob.MobSpawnManager.Instance.SpawnDungeonMonster(sd, nowMap.ID, nowMap.Map.ID, false, true);
                    else
                        //Mob.MobSpawnManager.Instance.LoadOne(dungeon.SpawnFile, nowMap.Map.ID, true, false);
                        Mob.MobSpawnManager.Instance.SpawnDungeonMonster(sd, nowMap.ID, nowMap.Map.ID, true, false);
