...
namespace SagaMap.ActorEventHandlers
{
    public class PCEventHandler : ActorEventHandler
    {
                ...
                case ActorType.FURNITURE:
                    {
                        ...
                        Item item = ItemFactory.Instance.GetItem(actor.ItemID);
                        if (this.Client.Map.ID > 90000000)
                        {
                            if (item.BaseData.itemType == ItemType.FF_CASTLE)
                            {
                                Packets.Server.SSMG_FF_CASTLE_APPEAR p3 = new Packets.Server.SSMG_FF_CASTLE_APPEAR();
            ...
            if (success)
            {
                if (Client.firstLogin)
                {
                    ...
                    if (Client.Character.Ring != null)
                        if (Client.Character.Ring.FF_ID != 0)
                            Client.SendRingFF();
                ...
                else
                {
                    ...
                    else if (Client.map.ID == 90001999 || Client.map.ID == 91000999)
                    {
                        Client.SendGotoFF();
        ...
        public void OnShowEffect(Actor aActor, MapEventArgs args)
        {
            ...
            this.Client.SendNPCShowEffect(arg.actorID, arg.x, arg.y, arg.effectID, arg.oneTime);
        }
        ...
        public void PropertyUpdate(UpdateEvent arg, int para)
        {
            switch (arg)
            {
                ...
                case UpdateEvent.WRP:
                    ...
                    break;
                case UpdateEvent.Frame:
                    if (para != 0)
                    {
                        if (para > 0)
                            Client.SendSystemMessage(string.Format("您获得了{0}点声望", para));
                        else
                            Client.SendSystemMessage(string.Format("您失去了{0}点声望", -para));
                        Client.SendSystemMessage(string.Format("您当前的声望为:{0}", Client.Character.Fame));
                    }
                    break;
            }
        }
