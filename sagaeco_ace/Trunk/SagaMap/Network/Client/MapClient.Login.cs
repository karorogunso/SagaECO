    ...
    public partial class MapClient
    {
        ...
        public void OnCharSlot(Packets.Client.CSMG_CHAR_SLOT p)
        {
            if (this.state == SESSION_STATE.AUTHENTIFICATED)
            {
                ...
                {
                    if (!chara.Rebirth)
                        ExperienceManager.Instance.CheckExp(this, LevelType.CLEVEL);
                    else
                        ExperienceManager.Instance.CheckExp(this, LevelType.CLEVEL2);
                    if (chara.JobJoint == PC_JOB.NONE)
                    {
                        if (map.Info.Flag.Test(SagaDB.Map.MapFlags.Dominion))
                            ExperienceManager.Instance.CheckExp(this, LevelType.JLEVEL2);
                        else
                        {
                            if (chara.Job == chara.JobBasic)
                                ExperienceManager.Instance.CheckExp(this, LevelType.JLEVEL);
                            else if (!chara.Rebirth)
                                ExperienceManager.Instance.CheckExp(this, LevelType.JLEVEL2);
                            else
                                ExperienceManager.Instance.CheckExp(this, LevelType.JLEVEL3);
                        }
                    }
                    else
                        ExperienceManager.Instance.CheckExp(this, LevelType.JLEVEL2);
                ...
                if (this.Character.Race != PC_RACE.DEM)
                {
                    if (!this.Character.Rebirth)
                        Manager.ExperienceManager.Instance.CheckExp(this, LevelType.CLEVEL, 0);
                    else
                        Manager.ExperienceManager.Instance.CheckExp(this, LevelType.CLEVEL2, 0);
                    if (this.Character.JobJoint == PC_JOB.NONE)
                    {
                        if (map.Info.Flag.Test(SagaDB.Map.MapFlags.Dominion))
                            Manager.ExperienceManager.Instance.CheckExp(this, LevelType.JLEVEL2, 0);
                        else
                        {
                            if (this.Character.Job == this.Character.JobBasic)
                                Manager.ExperienceManager.Instance.CheckExp(this, LevelType.JLEVEL, 0);
                            else if (!this.Character.Rebirth)
                                Manager.ExperienceManager.Instance.CheckExp(this, LevelType.JLEVEL2, 0);
                            else
                                Manager.ExperienceManager.Instance.CheckExp(this, LevelType.JLEVEL3, 0);
                        }
                    }
                    else
                        Manager.ExperienceManager.Instance.CheckExp(this, LevelType.JLEVEL2, 0);
