    ...
    public partial class MapClient
    {
        ...
        public void SendCL()
        {
            if (this.chara.Race == PC_RACE.DEM && this.state != SESSION_STATE.AUTHENTIFICATED)
            {
                ...
                {
                    p1.CurrentEP = this.Character.DominionEPUsed;
                    p1.EPRequired = (short)(ExperienceManager.Instance.GetEPRequired(this.chara) - this.chara.DominionEPUsed);
                    p1.CL = this.chara.DominionCL;
                }
                else
                {
        ...
        public void OnDEMCostLimitBuy(Packets.Client.CSMG_DEM_COST_LIMIT_BUY p)
        {
            if (demCLBuy)
            {
                ...
                if (this.Character.EP >= ep)
                {
                    ...
                    ExperienceManager.Instance.ApplyCL(this.Character, ep);
