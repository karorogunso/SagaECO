            ...
            if (accepted)
            {
                ...
                if (ringPartner != null)
                {
                    if (this.Character.Ring == null)
                    {
                        ...
                        {
                            p1.Resault = SagaMap.Packets.Server.SSMG_RING_INVITE_ANSWER_RESULT.Resaults.OK;
                            RingManager.Instance.AddMember(this.ringPartner.Ring, this.Character);
                        }
                        else
                            p1.Resault = SagaMap.Packets.Server.SSMG_RING_INVITE_ANSWER_RESULT.Resaults.MEMBER_EXCEED;
                    }
                    else
                    {
                        p1.Resault = SagaMap.Packets.Server.SSMG_RING_INVITE_ANSWER_RESULT.Resaults.ALREADY_IN_RING;
                    }
                }
                else
                {
                    p1.Resault = SagaMap.Packets.Server.SSMG_RING_INVITE_ANSWER_RESULT.Resaults.CANNOT_FIND_TARGET;
                }
                this.netIO.SendPacket(p1);
            }
            ...
            if (client != null)
            {
                if (this.Character.Ring != null)
                {
                    if (client.Character.Ring == null)
                    {
                        ...
                        {
                            p1.Resault = SagaMap.Packets.Server.SSMG_RING_INVITE_RESULT.Resaults.OK;
                        ...
                        {
                            p1.Resault = SagaMap.Packets.Server.SSMG_RING_INVITE_RESULT.Resaults.NO_RIGHT;
                        }
                    }
                    else
                    {
                        p1.Resault = SagaMap.Packets.Server.SSMG_RING_INVITE_RESULT.Resaults.TARGET_ALREADY_IN_RING;
                    }
                }
                else
                {
                    p1.Resault = SagaMap.Packets.Server.SSMG_RING_INVITE_RESULT.Resaults.NO_RING;
                }
            }
            else
            {
                p1.Resault = SagaMap.Packets.Server.SSMG_RING_INVITE_RESULT.Resaults.CANNOT_FIND_TARGET;
            }
            this.netIO.SendPacket(p1);
        }
