    ...
    public sealed class ExperienceManager : Singleton<ExperienceManager>
    {
        ...
        private void checkCL(ActorPC pc)
        {
            if (pc.Race == PC_RACE.DEM)
            {
                ...
                {
                    byte shouldLv = (byte)((pc.DominionCL - 9) / 4 + 1);
                    if (shouldLv > pc.DominionLevel)
                        SendLevelUp(MapClient.FromActorPC(pc), LevelType.CLEVEL, (uint)(shouldLv - pc.DominionLevel));
                }
                else
                {
        ...
        public void ApplyExp(ActorPC targetPC, ActorMob targetNPC, float percentage)
        {
            ...
            if (targetPC.CurrentJobLevel >= MaxJLevel)   // 修正EXP显示
            {
                targetjexp = 0;
            }
        ...
        public void CheckExp(MapClient client, LevelType type, byte when = 1)
        {
            ...
            switch (type)
            {
                case LevelType.CLEVEL:
                    if (client.map.Info.Flag.Test(SagaDB.Map.MapFlags.Dominion))
                    {
                        lvlDelta = this.GetLevelDelta(client.Character.DominionLevel, client.Character.DominionCEXP, LevelType.CLEVEL, true);
                    }
                    else
                ...
                case LevelType.JLEVEL2:
                    if (client.map.Info.Flag.Test(SagaDB.Map.MapFlags.Dominion))
                    {
                        lvlDelta = this.GetLevelDelta(client.Character.DominionJobLevel, client.Character.DominionJEXP, LevelType.JLEVEL2, true);
                        if (lvlDelta > 0)
                            this.SendLevelUp(client, LevelType.JLEVEL2, lvlDelta);
                    }
                    else
                    {
            ...
            if (type == LevelType.CLEVEL || type == LevelType.CLEVEL2)
            {
                ...
                if (client.map.Info.Flag.Test(SagaDB.Map.MapFlags.Dominion))
                {
                    if (client.Character.Race != PC_RACE.DEM)
                    {
                        ...
                        {
                            //stats += (ushort)((client.Character.DominionLevel + i) / 3 + 3);
                            stats += (ushort)(Math.Ceiling((float)(client.Character.Level + i) / 3.0f) + 2);
                        }
                        client.Character.DominionStatsPoint += stats;
                    }
