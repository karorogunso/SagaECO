...
namespace SagaMap.Skill.SkillDefinations.Eraser
{
    ...
    public class Instant : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            if (SkillHandler.Instance.CanAdditionApply(sActor, dActor, SkillHandler.DefaultAdditions.硬直, 50))
            {
                Additions.Global.Stiff skill = new SagaMap.Skill.Additions.Global.Stiff(args.skill, dActor, 3000);
                SkillHandler.ApplyAddition(dActor, skill);
            }
            SkillHandler.Instance.PhysicalAttack(sActor, new List<Actor>() { dActor }, args, SkillHandler.DefType.Def, SagaLib.Elements.Neutral, 0, factor, false, 0, false, 0, critbonus);
        }
    }
}
