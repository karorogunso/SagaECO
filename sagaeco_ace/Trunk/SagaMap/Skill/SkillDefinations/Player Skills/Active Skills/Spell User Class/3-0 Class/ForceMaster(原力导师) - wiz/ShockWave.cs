        ...
        public override void CallBack()
        {
            ...
            if (count <= countMax)
            {
                if (DistanceA <= 600)//If mob is out the range that FireBolt can cast, skip out.
                {
                    ...
                    SkillFireBolt.x = SagaLib.Global.PosX16to8(sActor.X, Manager.MapManager.Instance.GetMap(sActor.MapID).Width);
                    SkillFireBolt.y = SagaLib.Global.PosY16to8(sActor.Y, Manager.MapManager.Instance.GetMap(sActor.MapID).Height);
