...
namespace SagaMap.Skill
{
    public partial class SkillHandler : Singleton<SkillHandler>
    {
        ...
        public bool CanAdditionApply(Actor sActor, Actor dActor, string AdditionName, int rate)
        {
            ...
            if (dActor.Status.Additions.ContainsKey("Guardian"))
                newRate = 0;
            //是否可被賦予
            return (SagaLib.Global.Random.Next(1, 100) < newRate);
        }
        ...
        {
            //怪物抗性
            if (sActor.Ac_add_rate != 0)
                rate += sActor.Ac_add_rate;
            if (dActor.type == ActorType.MOB)
            {
        ...
        public void FixAttack(Actor sActor, List<Actor> dActor, DefType deftype, SkillArg arg, Elements element, float damage)
        {
            this.MagicAttack(sActor, dActor, arg, deftype, element, damage, 0, true);
        }
        /// <summary>
        /// 取得寵物
        /// </summary>
