...
namespace SagaMap.Tasks.PC
{
    public class CityDown : MultiRunTask
    {
        ...
        public override void CallBack()
        {
            ...
            try
            {
                if ((!client.Character.Status.Additions.ContainsKey("Heating") && client.map.Info.Cold) || 
                    (!client.Character.Status.Additions.ContainsKey("Aqualung") && client.map.Info.Wet) ||
                    (!client.Character.Status.Additions.ContainsKey("IntenseHeatSheld") && client.map.Info.Hot))
                {
