    ...
    public abstract class HasItemDetail : Packet
    {
        ...
        protected Item ItemDetail
        {
            set
            {
                ...
                this.PutInt(0);//price 商品の値段（露天商）（上の方のpriceの値と一致するとは限らない
                this.PutShort(0);//num 販売個数（露天商）（上の方の個数と一緒?
                this.PutInt(0);//unknown8
                this.PutShort(0);//unknown9
                //Logger.ShowError(string.Format("mapitem:{0}", value.ChangeMode));
                this.PutShort(1);//name_length
                this.PutByte(1);//name
