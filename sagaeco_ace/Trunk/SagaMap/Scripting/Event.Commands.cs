...
namespace SagaMap.Scripting
{
    public abstract partial class Event
    {
        ...
        /// <param name="pc">玩家</param>
        /// <param name="skillID">技能ID</param>
        /// <param name="IgnoreSkillList">是否无视职业技能列表</param>
        protected void LearnSkill(ActorPC pc, uint skillID, bool IgnoreSkillList)
        {
            if (IgnoreSkillList)
            {
                SagaDB.Skill.Skill skill = SkillFactory.Instance.GetSkill(skillID, 1);
        ...
        /// <param name="skillID">技能ID</param>
        protected void LostSkill(ActorPC pc, uint skillID)
        {
            SagaDB.Skill.Skill skill = SkillFactory.Instance.GetSkill(skillID, 1);
            ...
            if (pc.Skills.ContainsKey(skillID))
                pc.Skills.Remove(skillID);

            if (pc.Skills2.ContainsKey(skillID))
                pc.Skills2.Remove(skillID);

            if (pc.Skills2_1.ContainsKey(skillID))
                pc.Skills2_1.Remove(skillID);

            if (pc.Skills2_2.ContainsKey(skillID))
                pc.Skills2_2.Remove(skillID);

            if (pc.Skills3.ContainsKey(skillID))
                pc.Skills3.Remove(skillID);
        ...
        {
            NPCMove(pc, npcID, x, y, speed, 0, type, motion, motionSpeed);
        }
        protected void NPCMove(ActorPC pc, uint npcID, byte x, byte y, ushort speed, byte dir, byte type, ushort motion, ushort motionSpeed)
        {
