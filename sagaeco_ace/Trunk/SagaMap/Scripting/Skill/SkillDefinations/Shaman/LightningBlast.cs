    ...
    public class LightningBlast:ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            SkillHandler.Instance.MagicAttack(sActor, affected, args, SagaLib.Elements.Wind, factor);
            {
                ...
                {
                    if (SagaLib.Global.Random.Next(0, 100) < 15 * ((ActorPC)sActor).Skills[3041].Level)
                    {
                        foreach (Actor i in affected)
                        {
                            Additions.Global.Stun skill = new SagaMap.Skill.Additions.Global.Stun(args.skill, i, 2000);
