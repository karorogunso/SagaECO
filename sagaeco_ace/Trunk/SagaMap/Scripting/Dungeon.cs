    ...
    public class DungeonExit : Event
    {
        ...
        public override void OnEvent(ActorPC pc)
        {
            ...
            if (map.IsDungeon)
            {
                ...
                List<Dungeon.DungeonMap> maplst = Dungeon.DungeonFactory.Instance.GetDungeon(pc).Maps;
                foreach (var item in maplst)
                {
                    Actor act = item.Map.Actors.FirstOrDefault(x => x.Value.type == ActorType.PC).Value;
                    if (act != null)
                        return;
                }
                Dungeon.DungeonFactory.Instance.GetDungeon(pc).Destory(Dungeon.DestroyType.TimeOver);
            }
        }
    }
}
