...
{
    public class S910000110 : Event
    {
        public S910000110()
        {
            this.EventID = 910000110;
        }

        public override void OnEvent(ActorPC pc)
        {
            if (CountItem(pc, 910000110) >= 1)
            {
                TakeItem(pc, 910000110, 1);
                奖励(pc);
            }
        }
        ...
        void 奖励(ActorPC pc)
        {
            ...
            if (Global.Random.Next(0, 100) < 5)
            {
                ...
                if (pc.Party != null)
                {
                    ...
                    {
                        if (m.Value.Online)
                            SagaMap.Network.Client.MapClient.FromActorPC(m.Value).SendSystemMessage(pc.Name + " 从【迎春僵尸盒子】中获得了 " + item.BaseData.name);
                    }
                }
            }
