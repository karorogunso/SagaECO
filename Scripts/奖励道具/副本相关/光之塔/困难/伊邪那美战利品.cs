...
{
    public class S910000056 : Event
    {
        public S910000056()
        {
            this.EventID = 910000056;
        }

        public override void OnEvent(ActorPC pc)
        {
            if (CountItem(pc, 910000056) >= 1 && CountItem(pc, 950000016) >= 1)
            {
                TakeItem(pc, 950000016, 1);
                TakeItem(pc, 910000056, 1);
                奖励(pc);
            }
            else
            {
            ...
            if (Global.Random.Next(0, 100) < 20)
            {
                ...
                if (pc.Party != null)
                {
                    ...
                    foreach (var m in pc.Party.Members)
                        if (m.Value.Online)
                            SagaMap.Network.Client.MapClient.FromActorPC(m.Value).SendSystemMessage(pc.Name + " 从【伊邪那美战利品】中获得了 " + item.BaseData.name);
                }
            }
            if (Global.Random.Next(0, 100) < 20)
            {
                ...
                foreach (var m in pc.Party.Members)
                    if (m.Value.Online)
						if (item.Name == "")
							...
							SagaMap.Network.Client.MapClient.FromActorPC(m.Value).SendSystemMessage(pc.Name + " 从【伊邪那美战利品】中获得了 " + item.Name);
            }
            GiveItem(pc, item);
        }
    }
