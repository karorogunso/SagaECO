...
{
    public class S910000054 : Event
    {
        public S910000054()
        {
            this.EventID = 910000054;
        }

        public override void OnEvent(ActorPC pc)
        {
            if (CountItem(pc, 910000054) >= 1)
            {
                TakeItem(pc, 910000054, 1);
                //奖励(pc);
            }
        }
        void 奖励(ActorPC pc)
            ...
            if (Global.Random.Next(0, 100) < 60)
            {
                ...
                if (pc.Party != null)
                {
                    ...
                    foreach (var m in pc.Party.Members)
                        if (m.Value.Online)
                            SagaMap.Network.Client.MapClient.FromActorPC(m.Value).SendSystemMessage(pc.Name + " 从【人造体战利品】中获得了 " + item.BaseData.name);
                }
            }
