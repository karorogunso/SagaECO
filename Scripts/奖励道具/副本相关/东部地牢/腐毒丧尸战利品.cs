...
{
    public class S910000118 : Event
    {
        public S910000118()
        {
            this.EventID = 910000118;
        }

        public override void OnEvent(ActorPC pc)
        {
            if (CountItem(pc, 910000118) >= 1)
            {
                TakeItem(pc, 910000118, 1);
                奖励(pc);
            }
        }
        ...
        void 奖励(ActorPC pc)
        {
            ...
            int g = Global.Random.Next(2300, 9000);
            pc.Gold += g;
            ...
            if (Global.Random.Next(0, 100) < 101)
            {
                ...
                if (pc.Party != null)
                {
                    ...
                    foreach (var m in pc.Party.Members)
                        if (m.Value.Online)
                            SagaMap.Network.Client.MapClient.FromActorPC(m.Value).SendSystemMessage(pc.Name + " 从【腐毒丧尸战利品】中获得了 " + item.BaseData.name);
                }
            }
            ...
            if (Global.Random.Next(0, 100) < 26)
            {
                uint id = 960000000;
                PlaySound(pc, 2040, false, 100, 50);
