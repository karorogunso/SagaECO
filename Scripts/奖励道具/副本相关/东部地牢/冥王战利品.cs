...
{
    public class S910000120 : Event
    {
        public S910000120()
        {
            this.EventID = 910000120;
        }

        public override void OnEvent(ActorPC pc)
        {
            if (CountItem(pc, 910000120) >= 1)
            {
                TakeItem(pc, 910000120, 1);
                奖励(pc);
            }
        }
        ...
        void 奖励(ActorPC pc)
        {
            ...
            int g = Global.Random.Next(3000, 9000);
            pc.Gold += g;
            ...
            if (Global.Random.Next(0, 100) < 101)
            {
                ...
                if (pc.Party != null)
                {
                    ...
                    foreach (var m in pc.Party.Members)
                        if (m.Value.Online)
                            SagaMap.Network.Client.MapClient.FromActorPC(m.Value).SendSystemMessage(pc.Name + " 从【冥王战利品】中获得了 " + item.BaseData.name);
                }
            }
            ...
            if (Global.Random.Next(0, 100) < 28)
            {
                uint id = 960000000;
                PlaySound(pc, 2040, false, 100, 50);
