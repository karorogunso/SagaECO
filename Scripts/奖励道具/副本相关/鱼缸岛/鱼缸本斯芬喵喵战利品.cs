...
{
    public class S910000073 : Event
    {
        public S910000073()
        {
            this.EventID = 910000073;
        }

        public override void OnEvent(ActorPC pc)
        {
            if (CountItem(pc, 910000073) >= 1)
            {
                TakeItem(pc, 910000073, 1);
                奖励(pc);
            }
        }
        ...
        void 奖励(ActorPC pc)
        {
            ...
            if (Global.Random.Next(0, 100) < 15)
            {
                ...
                if (pc.Party != null)
                {
                    ...
                    {
                        if (m.Value.Online)
                            SagaMap.Network.Client.MapClient.FromActorPC(m.Value).SendSystemMessage(pc.Name + " 从【鱼缸本斯芬喵喵战利品】中获得了 " + item.BaseData.name);
                    }
                }
            }
