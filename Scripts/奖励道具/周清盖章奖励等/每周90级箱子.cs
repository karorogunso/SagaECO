...
{
    public class S910000022 : Event
    {
        public S910000022()
        {
            this.EventID = 910000022;
        }

        public override void OnEvent(ActorPC pc)
        {
            if (CountItem(pc, 910000022) > 0)
            {
                ...
                GiveItem(pc, 940000001, 1);//+9~+11强化保护石
                GiveItem(pc, 950000001, 1);//脸币
                GiveItem(pc, 950000000, 3);//发型币
                GiveItem(pc, 950000002, 1);//扭曲石头
                pc.Gold += Global.Random.Next(100000, 800000);
                GiveItem(pc, 100000000, 300);//KUJI币
                GiveItem(pc, 950000005, 50);//特质KUJI币
                GiveItem(pc, 950000003, 150);//武器碎片

                GiveItem(pc, 910000040, 2);//CP5000

                TakeItem(pc, 910000022, 1);
            }
        }
    }
}
