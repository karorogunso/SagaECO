...
{
    public class S950000031 : Event
    {
        public S950000031()
        {
            this.EventID = 950000031;//金色钥匙ID为950000031
        }

        public override void OnEvent(ActorPC pc)
        {
            if (CountItem(pc, 950000031) >= 1)
            {
                ...
                for (int i = 0; i <myKujis.Count; i++)//循环myKujis.Count
                {
                    ...
                    if (kuji.BaseData.itemType == ItemType.PARTNER)
                    {
                        SagaDB.Partner.PartnerData p = SagaDB.Partner.PartnerFactory.Instance.GetPartnerData(kuji.BaseData.petID);
                        if (p.base_rank == 101)
                        {
                            kujiname += "[无法兑换]";
                        }
                ...
                Say(pc, 131, "金色钥匙无法兑换SSS级搭档，$R但可以兑换SSS盒子内的其他物品。", "金钥匙！");
                int set = Select(pc, "请选择要用金色钥匙开启的盒子", "" ,ops.ToArray());//弹出选项
                ...
                SagaDB.Item.Item Bitem = ItemFactory.Instance.GetItem(BSetItemID);
                if(Bitem.BaseData.itemType == ItemType.PARTNER)
                {
                    //Say(pc, 131, Bitem.BaseData.petID.ToString());
                    SagaDB.Partner.PartnerData p = SagaDB.Partner.PartnerFactory.Instance.GetPartnerData(Bitem.BaseData.petID);
                    if(p.base_rank == 101)
                    {
                        Say(pc, 131, "金钥匙无法兑换SSS搭档哦。","您在使用金钥匙");
                        return;
                    }
                }
                if (CountItem(pc, 950000031) >= 1 && CountItem(pc,SetkujiID) >= 1
                    && Select(pc,"真的要拿「" + Bitem.BaseData + "」吗？","","是的","算了") ==1)//给奖励前判断一次钥匙，防止有玩家选择之前把钥匙消灭 和是否拥有KUJI盒子
                {
                    GiveItem(pc, BSetItemID, 1);//给奖励！
                    TakeItem(pc, 950000031, 1);
