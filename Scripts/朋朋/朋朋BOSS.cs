...
{
    public partial class S50003013
    {
        public void 刷朋朋(ActorPC pc, uint mapid)
        {
            SagaMap.Map map = SagaMap.Manager.MapManager.Instance.GetMap(mapid);
            ActorMob mob = map.SpawnCustomMob(10000000, mapid, 16200000, 0, 0, 0, 59, 61, 1, 1, 0, 迷你兔Info(), 迷你兔AI(), null, 0)[0];
            ...
            {
                pc.Party.Leader.TInt["S60913000"] = 0;
                    ...
                    foreach (var item in pc.Party.Members.Values)
                    {
                        if (item.type == ActorType.PC)
                        {
                            ...
                            if (m.Online && m != null)
                            {
                                ...
                                int time = (int)(DateTime.Now - pc.Party.TTime["朋朋时间"]).TotalSeconds;
                                ...
                                SagaMap.Network.Client.MapClient.FromActorPC(item).SendSystemMessage("通关时间：" + time.ToString() + "秒");
                                if (DateTime.Now < DateTime.Parse("2016-10-18"))
                                {
                                    ActorPC server = SagaMap.Manager.ScriptManager.Instance.VariableHolder;
                                    if (server.Adict["朋朋通关_排行榜"]["朋朋通关_" + m.Account.AccountID.ToString()] == 0)
                                        server.Adict["朋朋通关_排行榜"]["朋朋通关_" + m.Account.AccountID.ToString()] = time;
                                    else if (time < server.Adict["朋朋通关_排行榜"]["朋朋通关_" + m.Account.AccountID.ToString()])
                                        server.Adict["朋朋通关_排行榜"]["朋朋通关_" + m.Account.AccountID.ToString()] = time;
                                }
                                SStr["朋朋通关_" + m.Account.AccountID.ToString()] = m.Name;
                            }
                        }
                    }
                }
        ...
        ActorMob.MobInfo 迷你兔Info()
        {
            ...
            info.jobExp = info.maxhp / 3;//職業經驗值
