...
{
    public class S953000014 : Event
    {
        public S953000014()
        {
            this.EventID = 953000014;
        }

        public override void OnEvent(ActorPC pc)
        {
            if (CountItem(pc, 953000014) >= 1)
            {
                TakeItem(pc, 953000014, 1);
                奖励(pc);
            }
        }
        void 奖励(ActorPC pc)
        {
            ...
            int g = Global.Random.Next(20000, 30000);
            pc.Gold += g;
            if (Global.Random.Next(0, 100) < 100)
            {
                ...
                if (pc.Party != null)
                {
                    ...
                    foreach (var m in pc.Party.Members)
                        if (m.Value.Online)
                            SagaMap.Network.Client.MapClient.FromActorPC(m.Value).SendSystemMessage(pc.Name + " 从【常暗战利品】中获得了 " + item.BaseData.name);
                }
            }
