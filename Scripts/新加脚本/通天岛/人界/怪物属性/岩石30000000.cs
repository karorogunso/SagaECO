...
{
    public partial class 通天岛
    {
        ActorMob.MobInfo 岩石Info()
        {
            ...
            newDrop.ItemID = 10014600;//掉落物品ID
            newDrop.Rate = 10000;//掉落幾率,10000是100%，5000是50%
            //newDrop.count = (ushort)Global.Random.Next(1, 3);
newDrop.MinCount = 1;
newDrop.MaxCount = 3;
            info.dropItems.Add(newDrop);

            newDrop = new MobData.DropData();
            newDrop.ItemID = 10014800;//掉落物品ID
            newDrop.Rate = 2000;//掉落幾率,10000是100%，5000是50%
            newDrop.count = 1;
            info.dropItems.Add(newDrop);

            newDrop = new MobData.DropData();
            newDrop.ItemID = 10017300;//掉落物品ID
