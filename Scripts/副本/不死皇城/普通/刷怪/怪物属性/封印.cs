...
namespace WeeklyExploration
{
    public partial class GQuest : Event
    {
        ...
        ActorMob.MobInfo 封印Info()
        {
            ActorMob.MobInfo info = new ActorMob.MobInfo();
            info.maxhp =(uint)(1.5 *  20000000);//血量
            info.name = "封印";
            info.speed = 0;//移動速度
            info.atk_min = 0;//最低物理攻擊
            info.atk_max = 0;//最高物理攻擊
            info.matk_min = 0;//最低魔法攻擊
            info.matk_max = 1;//最高物理攻擊
            info.def = 0;//物理左防
            info.mdef = 0;//魔法左防
            info.def_add = 0;//物理右防
            info.mdef_add = 0;//魔法右防
            info.hit_critical = 0;//暴擊值
            info.hit_magic = 0;//魔法命中值（目前沒用
            info.hit_melee = 0;//近戰命中值
            info.hit_ranged = 0;//遠程命中值
            ...
            info.baseExp = (uint)(0.5 * 100);//基礎經驗值
            info.jobExp = (uint)(0.5 * 100);//職業經驗值
        ...
        AIMode 封印AI()
        {
            AIMode ai = new AIMode(4); ai.MobID = 10000000; ai.isNewAI = true;//1為主動，0為被動
