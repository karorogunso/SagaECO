...
namespace WeeklyExploration
{
    public partial class GQuest : Event
    {
        ...
        void 光之塔老一刷怪困难(uint mapid, ActorPC pc)
        {
            SagaMap.Map map;
            map = SagaMap.Manager.MapManager.Instance.GetMap(mapid);
            ActorMob mob = map.SpawnCustomMob(10000000, map.ID, 19010157, 0, 0, 77, 74, 0, 1, 0, 老一Info困难(), 老一AI困难(), (MobCallback)BOSS1死亡Ondie, 1)[0];
            ((MobEventHandler)mob.e).Defending += SQuest_Defending困难;
            ((MobEventHandler)mob.e).Returning += SQuest_Returning困难;
            pc.Party.Leader.TInt["副本BOSSID"] = (int)mob.ActorID;

        }

        private void SQuest_Returning困难(MobEventHandler eh, ActorPC pc)
        {
        ...
        private void SQuest_Defending困难(MobEventHandler eh, ActorPC pc)
        {
            if (eh.mob.AttackedForEvent == 0)
            {
                ActorMob mob = eh.mob;
            ...
            else if (eh.mob.AttackedForEvent == 3)
            {
                ...
                if (mob.HP < 500)
                {
                    ...
                    光之塔老二刷怪困难((uint)pc.Party.Leader.TInt["S20146000"], pc);
