...
namespace SSPEditor
{
    ...
    public class SkillData : INotifyPropertyChanged, IComparable<SkillData>  //通知 & 可排序
    {
        ...
        {
            return (SkillData)MemberwiseClone();//object类的MemberwiseClone();实现克隆。由于是保护的方法，所以需要给自己的类添加Clone方法间接调用而不能直接使用。
        }
    ...
    public partial class MainWindow : Window
    {
        ...
        public static extern bool DeleteObject(IntPtr hObject); //载入图标时，将BitMap转为image控件使用的Source后，需要使用DeleteObject施放内存。
