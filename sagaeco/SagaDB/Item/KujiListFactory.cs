...
namespace SagaDB.Item
{
    public class KujiListFactory : Singleton<KujiListFactory>
    {
        ...
        public List<uint> FurnitureList = new List<uint>();
        public List<uint> Item9List = new List<uint>();
        public Dictionary<uint, List<Kuji>> KujiList { get { return kujilist; } }

        public Dictionary<uint, uint> ItemChoiceList = new Dictionary<uint, uint>();

        public Dictionary<uint, string> SpecialSkillNames = new Dictionary<uint, string>();
        ...
        public void InitChoiceList(string path, Encoding encoding)
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                try
                {
                    ...
                    uint Price = uint.Parse(paras[2]);
                    if (!ItemChoiceList.ContainsKey(Itemid))
                        ItemChoiceList.Add(Itemid, Price);
                }
                catch (Exception ex)
                {
                    Logger.ShowError(ex);
        ...
        public void BuildNotInKujiItemsList()
        {
            ...
            foreach (var item in ItemFactory.Instance.Items)
            {
                ...
                if(ty == ItemType.FURNITURE || ty == ItemType.FF_ROOM_FLOOR || ty == ItemType.FG_GARDEN_FLOOR || ty == ItemType.FG_GARDEN_MODELHOUSE
                             || ty == ItemType.FG_ROOM_FLOOR || ty == ItemType.FG_ROOM_WALL || ty == ItemType.FG_FLYING_SAIL)
                    FurnitureList.Add(item.Key);
            }
        }

        public void InitItem9List(string path, Encoding encoding)
        {
            System.IO.StreamReader sr = new System.IO.StreamReader(VirtualFileSystemManager.Instance.FileSystem.OpenFile(path), encoding);
        ...
        {
            int[] rates = new int[9] { 3, 17, 80, 100, 160, 200, 260, 280,300};
            if (rank > 9)
            {
                rank = 300;
            }
