...
namespace SagaDB.Skill
{
    public class SkillFactory : Singleton<SkillFactory>
    {
        ...
        public Dictionary<PC_JOB, Dictionary<uint, byte>> skills = new Dictionary<PC_JOB, Dictionary<uint, byte>>();
        ...
        class PreconditionSkill
        {
            ...
            public Dictionary<string, uint> PreconditionVariable = new Dictionary<string, uint>();
        }
        class sspHeader
        {
            public uint offset;
        ...
        public Dictionary<uint, byte> CheckSkillList(ActorPC pc, SkillPaga paga,bool isread)
        {
            ...
            //if (paga == SkillPaga.p1) //获取1转技能页时候，要return1转技能+装备临时技能
            ...
            foreach (KeyValuePair<uint, PreconditionSkill> skill in skills2[pc.Job3])
            {
                ...
                else
                {
                    ...
                    {
                        if (!pc.Skills.ContainsKey(ps.Key) && !pc.Skills2_1.ContainsKey(ps.Key) && !pc.Skills2_2.ContainsKey(ps.Key) && !pc.Skills3.ContainsKey(ps.Key) && !pc.SkillsEquip.ContainsKey(ps.Key)) continue;
                        ...
                        if (pc.SkillsEquip.ContainsKey(ps.Key)) if (pc.SkillsEquip[ps.Key].Level < ps.Value) continue;
                        count++;
                    }
                    foreach (KeyValuePair<string, uint> ps in skill.Value.PreconditionVariable)
                    {
                        if (pc.CInt[ps.Key]<ps.Value) continue;
                        count++;
                    }
                    if (count == skill.Value.PreconditionSkillInfo.Count + skill.Value.PreconditionVariable.Count && skill.Value.LearnSkillJobLv <= pc.JobLevel3)
                        result.Add(skill.Key, (byte)skill.Value.LearnSkillJobLv);
                }
            }
            return result;
                                        ...
                                        foreach(object l2 in PreconditionSkills)
                                        {
                                            ...
                                            switch(k2.Name)
                                            {
                                                case "PreconditionSkill":
                                                    ...
                                                    break;
                                                case "PreconditionVariable":
                                                    uint PreconditionVariableVal = uint.Parse(k2.Attributes["Val"].InnerText);
                                                    string PreconditionVariableName = k2.InnerText;
                                                    if (!list2[LearnSkillID].PreconditionVariable.ContainsKey(PreconditionVariableName))
                                                        list2[LearnSkillID].PreconditionVariable.Add(PreconditionVariableName, PreconditionVariableVal);
                                                    break;
                                            }
                                        }
                                        break;
        ...
        public void InitSSP(string path, System.Text.Encoding encoding)
        {
            ...
            try
            {
                ...
                foreach (sspHeader i in header)
                {
                    ...
                    br.ReadUInt16();//another mark
                    if (skill.id == 0)
                        continue;
                    string buf = encoder.GetString(br.ReadBytes(116));//名称后12字节被设定为castTime、delay、singleCD
                ...
                Logger.ProgressBarHide(count + " skills loaded."+(DateTime.Now - time).TotalMilliseconds.ToString()+"ms");
                fs.Close();
                br.Close();
