        ...
        public ActorPC Player
        {
            set
            {
                            ...
                            if (appequips.ContainsKey((EnumEquipSlot)j) || equips.ContainsKey((EnumEquipSlot)j))
                            {
                                ...
                                if (j == 9 && item.EquipSlot.Contains(EnumEquipSlot.RIGHT_HAND) && item.EquipSlot.Count == 1)
                                {
                                    if (item.PictID == 0)
                                        ...
                                        pid = ItemFactory.Instance.GetItem(item.PictID).BaseData.imageID + 10000000;
                                ...
                                else
                                {
                                    ...
                                    if (item.PictID == 0)
                                        ...
                                        pid = ItemFactory.Instance.GetItem(item.PictID).BaseData.imageID;
                                    if (value.TInt["斥候远程模式"] == 1 && value.Job == PC_JOB.HAWKEYE && item.EquipSlot.Contains(EnumEquipSlot.RIGHT_HAND) && !item.BaseData.doubleHand) pid = 0;
                                    else if (value.TInt["斥候远程模式"] == 0 && value.Job == PC_JOB.HAWKEYE && item.EquipSlot.Contains(EnumEquipSlot.LEFT_HAND) && !item.BaseData.doubleHand) pid = 0;
                        ...
                        value.Marionette == null && value.TranceID == 0)
                {
                    ...
                    PutUShort(leftHand.HandMotion != 0 ? leftHand.HandMotion : leftHand.BaseData.handMotion, 64);
                    //this.offset += 2;
                    try
                    {
                        if (leftHand.HandMotion2 != 0)
                            PutUShort(leftHand.HandMotion2, 68);
                        else
                            PutUShort((ushort)(EquipSound)Enum.Parse(typeof(EquipSound), leftHand.BaseData.itemType.ToString()), 68);
                        ...
                        value.Marionette == null && value.TranceID == 0)
                {
                    ...
                    PutUShort(rightHand.HandMotion != 0 ? rightHand.HandMotion : rightHand.BaseData.handMotion, 71);
                    try
                    {
                        if (rightHand.HandMotion2 != 0)
                            PutUShort(rightHand.HandMotion2,  75);
                        else
                            PutUShort((ushort)(EquipSound)Enum.Parse(typeof(EquipSound), rightHand.BaseData.itemType.ToString()), 75);
