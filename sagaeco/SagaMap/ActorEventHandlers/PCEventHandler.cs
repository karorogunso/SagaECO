...
namespace SagaMap.ActorEventHandlers
{
    public class PCEventHandler : ActorEventHandler
    {
        ...
        public void OnActorAppears(Actor aActor)
        {
            ...
            MapInfo info;
            if (!this.Client.Character.VisibleActors.ContainsKey(aActor.ActorID))
                this.Client.Character.VisibleActors.Add(aActor.ActorID, aActor.ActorID);
            switch (aActor.type)
            {
                case ActorType.PC:
                ...
                case ActorType.FURNITURE:
                    {
                        ...
                        Item item = ItemFactory.Instance.GetItem(actor.ItemID);
                        if (this.Client.Map.ID > 90001000 && Client.map.OriID != 70000000 && Client.map.OriID != 75000000)
                        {
                            if (item.BaseData.itemType == ItemType.FF_CASTLE)
                            {
                                Packets.Server.SSMG_FF_CASTLE_APPEAR p3 = new Packets.Server.SSMG_FF_CASTLE_APPEAR();
        ...
        {
            if (Client == null) return;
            if (Client.Character.VisibleActors.ContainsKey(dActor.ActorID))
                Client.Character.VisibleActors[dActor.ActorID] = dActor.ActorID;
            switch (dActor.type)
            {
                case ActorType.PC:
        ...
        {
            if (Client == null) return;
            if (!Client.Character.VisibleActors.ContainsKey(mActor.ActorID) && mActor.ActorID != this.Client.Character.ActorID)
            {
        ...
        public void OnCreate(bool success)
        {
            ...
            if (success)
            {
                ...
                else
                {
                    ...
                    else if (Client.map.ID == 90001999 || Client.map.ID == 10054000)
                    {
                        Client.SendGotoFF();
                ...
                if (Client.Character.MapID == 90001999 || Client.map.OriID == 70000000)
        ...
        public void OnActorChangeMotion(Actor aActor, MapEventArgs args)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    p.Motion = arg.motion;
                    p.Loop = arg.loop;
                    //if (aActor.type == ActorType.PC && !arg.send) return; 
                    Client.netIO.SendPacket(p);
                }
            }
        }
        ...
        public void OnCharInfoUpdate(Actor aActor)
        {
            ...
            if (aActor.type == ActorType.PC)
            {
                ...
                MapClient.FromActorPC(aActor as ActorPC).SendRange();
        ...
        public void OnDie()
        {
            ...
            if (Client.Character.Status.Additions.ContainsKey("沙月剧毒丸伤害提升") && !Client.Character.Status.Additions.ContainsKey("自杀CD"))
                Client.TitleProccess(Client.Character, 142, 1);

            Client.Character.ClearTaskAddition();

            /*if (!Client.Character.Tasks.ContainsKey("SpRecover"))
            {
                Tasks.PC.SpRecover task = new SagaMap.Tasks.PC.SpRecover(Client);
                Client.Character.Tasks.Add("SpRecover", task);
                task.Activate();
            }*/
            Client.Character.Buff.恶炎 = false;
            ...
            Skill.SkillHandler.Instance.RandomEquipWorn(this.Client.Character);
            ...
            if (Client.Character.Status.Penglai > 0 && Client.Character.Job == PC_JOB.ASTRALIST && DateTime.Now > Client.Character.TTime["蓬莱之烟CD"])
            {
                int lifetime = 30 -  (2 * Client.Character.Status.Penglai);
                Client.Character.TTime["蓬莱之烟CD"] = DateTime.Now + new TimeSpan(0, 0, lifetime);
                Client.Character.BattleStatus = 0;
                Client.SendChangeStatus();
                Client.EventActivate(70182030);
            }

            if(Client.Character.Skills.ContainsKey(20016) && Client.Character.Status.Additions.ContainsKey("神佑：返魂CD"))
            {
                Client.Character.BattleStatus = 0;
                Client.SendChangeStatus();
                Client.EventActivate(70182031);
            }
        ...
        public void OnAttack(Actor aActor, MapEventArgs args)
        {
            ...
            {
                //Logger.ShowWarning(arg.result.ToString());
        ...
        {
            if (Client == null) return;
            if (Client.Character.TInt["屏蔽特效"] == 2) return;
            EffectArg arg = (EffectArg)args;
            if (Client.Character.TInt["屏蔽特效"] == 1 && arg.caster != null)
            {
                if (arg.caster.ActorID != Client.Character.ActorID)
                    return;
            }
        ...
        public void PropertyUpdate(UpdateEvent arg, int para)
        {
            ...
            switch (arg)
            {
                ...
                case UpdateEvent.QUEST_POINT:
                    ...
                    if (para > 0)
                        Client.SendSystemMessage("增加了 " + para + " 点任务点.");
                    else
                        Client.SendSystemMessage("失去了 " + -para + " 点任务点.");
                    break;
                case UpdateEvent.MOTION:
                    ChatArg parg = new ChatArg();
                    parg.motion = Client.Character.Motion;
                    parg.loop = Client.Character.MotionLoop ? (byte)1 : (byte)0;
                    parg.send = true;
                    Client.map.SendEventToAllActorsWhoCanSeeActor(Map.EVENT_TYPE.MOTION, parg, Client.Character, true);
                    if (parg.loop != 1 && parg.motion != MotionType.SIT)
                        Client.Character.Motion = 0;
                    if (Client.Character.Motion != MotionType.SIT)
                    {
                        Client.Character.Buff.Sit = false;
        ...
        public void OnActorFurnitureList(Object obj)
        {
            ...
            {
                Packets.Server.SSMG_FG_ACTOR_APPEAR_LIST p1 = new SagaMap.Packets.Server.SSMG_FG_ACTOR_APPEAR_LIST();
                p1.MapID = this.Client.Map.ID;
                p1.fs = (List<ActorFurniture>)obj;
                Client.netIO.SendPacket(p1);
            }
            /*
            if (obj is List<ActorFurniture>)
