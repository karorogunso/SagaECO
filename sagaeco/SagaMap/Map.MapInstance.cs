    ...
    public partial class Map
    {
        ...
        public void OnDestrory()
        {
            ...
            foreach (Actor i in pcs)
            {
                ...
                {
                    ActorPC p = (ActorPC)i;
                    Network.Client.MapClient.FromActorPC(p).RevivePC(p);
                    SendActorToMap(i, clientExitMap, Global.PosX8to16(clientExitX, map.Width), Global.PosY8to16(clientExitY, map.Height));
                }
                catch(Exception ex)
                {
