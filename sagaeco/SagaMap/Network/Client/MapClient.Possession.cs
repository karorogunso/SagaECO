...
namespace SagaMap.Network.Client
{
    public partial class MapClient
    {
        ...
        {
            if (this.Character.MapID != 10054000 && Character.Account.GMLevel < 200)
            {
                this.SendSystemMessage("目前只可以在鱼缸岛城寨凭依哦~");
                return;
            }
        ...
        int TestPossesionPosition(ActorPC target, PossessionPosition pos)
        {
            ...
            if (this.Character.Buff.憑依準備 == true || this.Character.PossessionTarget != 0)
            {
                return -1;
            }
            if (target.Buff.憑依準備 == true)
            {
                return -17;
            }
