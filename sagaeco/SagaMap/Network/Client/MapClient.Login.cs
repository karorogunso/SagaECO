    ...
    public partial class MapClient
    {
        ...
        public void OnLogin(Packets.Client.CSMG_LOGIN p)
        {
            ...
            if (MapServer.accountDB.CheckPassword(p.UserName, p.Password, this.frontWord, this.backWord))
            {
                ...
                {
                    if(account.LastIP == item.account.LastIP)
                    {
                        checkdk = true;
                        dkip = account.LastIP;
                    }
                ...
                if (checkdk)
                {
                    Logger log = new Logger("多开检查.txt");
                    string logtext = "IP地址：" + dkip + "\r\n";
                    foreach (var item in MapClientManager.Instance.OnlinePlayer)
                    {
                        if (account.LastIP == item.account.LastIP)
                            logtext += "-玩家账号：" + item.account.Name + "|角色名：" + item.account.PlayerNames + "|Mac：" + item.account.MacAddress + "\r\n";
                    }
                    logtext += "*玩家账号：" + account.Name + "|角色名：" + account.PlayerNames + "|Mac：" + account.MacAddress + "\r\n";
                    logtext += "\r\n=======================================================\r\n";
                ...
                if (ScriptManager.Instance.VariableHolder.AStr["多开MAC限制时间"] != DateTime.Now.ToString("yyyy-MM-dd"))
                {
                    ...
                    ScriptManager.Instance.VariableHolder.Adict["多开MAC限制"] = new VariableHolderA<string, int>();
                    ScriptManager.Instance.VariableHolder.Adict["多开当日限制登录的账号"] = new VariableHolderA<string, int>();
                }
        ...
        public void OnCharSlot(Packets.Client.CSMG_CHAR_SLOT p)
        {
            if (this.state == SESSION_STATE.AUTHENTIFICATED)
            {
                    ...
                    if (this.Character.PossessionTarget < 10000)
                    {
                        ...
                        if (client != null)
                        {
                            ...
                            if (found != null)
                            {
                                ...
                                {
                                    if (Character.MapID != 30201002)
                                    {
                                        SagaDB.Map.MapInfo info = SagaDB.Map.MapInfoFactory.Instance.MapInfo[30201002];
                                        byte x = (byte)SagaLib.Global.Random.Next(3, 9);
                                        byte y = (byte)SagaLib.Global.Random.Next(4, 11);

                                        this.Character.MapID = 30201002;
                                        this.Character.X = SagaLib.Global.PosX8to16(x, info.width);
                                        this.Character.Y = SagaLib.Global.PosY8to16(y, info.height);
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        SagaDB.Map.MapInfo info = SagaDB.Map.MapInfoFactory.Instance.MapInfo[10054000];
                                        byte x = (byte)SagaLib.Global.Random.Next(196, 198);
                                        byte y = (byte)SagaLib.Global.Random.Next(164, 166);

                                        this.Character.MapID = 10054000;
                                        this.Character.X = SagaLib.Global.PosX8to16(x, info.width);
                                        this.Character.Y = SagaLib.Global.PosY8to16(y, info.height);
                                    }
                                }
                ...
                if (this.map == null)
                {
                    ...
                    {
                        this.Character.SaveMap = 10054000;
                        this.Character.SaveX = 197;
                        this.Character.SaveY = 165;
                    }
                ...
                //附加从数据库读取的队列BUFF列表
                foreach (var item in Character.PendingAddtions)
                {
                    if (!Character.Status.Additions.ContainsKey(item.Key) && item.Value > DateTime.Now)
                    {
                        int lifetime = (int)(item.Value - DateTime.Now).TotalMilliseconds;
                        if (Skill.SkillHandler.Instance.StableBuffsHandler.ContainsKey(item.Key))
                            Skill.SkillHandler.Instance.StableBuffsHandler[item.Key].ApplyBuff(Character, lifetime);
                    }
                }
                /*
        ...
        public void OnMapLoaded(Packets.Client.CSMG_PLAYER_MAP_LOADED p)
        {
            ...
            if (this.Character.MapID == 90001999 || this.map.OriID == 70000000 || this.Character.MapID == 10054000)
                CustomMapManager.Instance.EnterFFOnMapLoaded(this);
            if(map.OriID == 70000000 || map.OriID == 75000000)
            {
                if (map.Creator != null && map.Creator.CInt["FGBGM"] > 0)
                {
                    Packets.Server.SSMG_NPC_CHANGE_BGM p3 = new Packets.Server.SSMG_NPC_CHANGE_BGM();
                    p3.SoundID = (uint)map.Creator.CInt["FGBGM"];
                    p3.Loop = 50;
                    p3.Volume = 100;
                    this.netIO.SendPacket(p3);
                }
