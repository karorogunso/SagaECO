...
namespace SagaMap.Network.Client
{
    public partial class MapClient
    {
        ...
        public void OnTradeRequest(Packets.Client.CSMG_TRADE_REQUEST p)
        {
            ...
            {
                client.SendSystemMessage("非常抱歉，为防止多开效益，交易系统已经关闭。");
        ...
        long GetGoldForRecycle(List<uint> tradeItems, List<ushort> tradeCounts)
        {
            ...
            {
                Item item = Character.Inventory.GetItem(tradeItems[i]).Clone();
                if (item.Stack < tradeCounts[i])
                {
                    ...
                    string logtext = "\r\n" + Character.Name + "使用了交易，修改了数量：" + tradeCounts[i] + "/" + item.Stack;
        ...
        {
            this.npcTradeItem = new List<Item>();
            List<uint> tItems = new List<uint>();
            List<ushort> tCounts = new List<ushort>();
            List<uint> ect_wespons = new List<uint>() { 160088200, 160088300, 160088400, 160088900, 160089000, 160088500, 160088600 };
            this.SendTradeEnd(2);
            if (this.tradeItems != null)
            {
                for (int i = 0; i < tradeItems.Count; i++)
                {
                    ...
                    bool canTrade = true;
                    if (item.BaseData.itemType == ItemType.EFFECT || item.BaseData.itemType == ItemType.PARTNER || item.PictID != 0 || item.Protecteditem == true)
                    {
                        if (Character.TInt["垃圾箱记录"] == 1)
                        {
                            canTrade = false;
                            SendSystemMessage("道具『" + item.Name + "』无法被回收垃圾桶。");
                        }
                    }
                    if(canTrade)
                    {
                        npcTradeItem.Add(item);
                        tItems.Add(tradeItems[i]);
                        tCounts.Add(tradeCounts[i]);
                    }
                }
            }
            if (Character.TInt["垃圾箱记录"] == 1)
                tradingGold = GetGoldForRecycle(tItems, tCounts);
            for (int i = 0; i < tItems.Count; i++)
            {
                Item item = Character.Inventory.GetItem(tItems[i]).Clone();
                DeleteItem(tItems[i], tCounts[i], true);
                Logger.LogItemLost(Logger.EventType.ItemNPCLost, this.Character.Name + "(" + this.Character.CharID + ")", item.BaseData.name + "(" + item.ItemID + ")", string.Format("NPCTrade Count:{0}", item.Stack), false);
            }
            ...
            if (Character.TInt["垃圾箱记录"] == 1)
            {
                ...
                Character.AInt["垃圾桶累积兑换CP"] += (int)tradingGold;
                string name = "称号125完成度";
                Character.AInt[name] = Character.AInt["垃圾桶累积兑换CP"];
                Title t = TitleFactory.Instance.TitleList[125];
                if (Character.AInt[name] >= t.ConCount)
                    UnlockTitle(Character, 125);
            }
