...
namespace SagaMap.Network.Client
{
    public partial class MapClient
    {
        ...
        public void OnNPCChoiceBuy(Packets.Client.CSMG_NPC_CHOICE_BUY p)
        {
            uint[] goods = p.Goods;
            ...
            {
                if (KujiListFactory.Instance.ItemChoiceList.ContainsKey(goods[i]))
                {
                    uint price = KujiListFactory.Instance.ItemChoiceList[goods[i]];
                    uint count = counts[i];
                    long tgold = price * count;
                    if(Character.Gold < tgold)
                    {
                        SendSystemMessage("钱不够啦");
                        return;
                    }
                }
            }
            for (int i = 0; i < goods.Length; i++)
            {
                if (KujiListFactory.Instance.ItemChoiceList.ContainsKey(goods[i]))
                {
                    ...
                    item.Stack = counts[i];
                    if (item.BaseData.itemType == ItemType.EFFECT)
                    {
                        SendSystemMessage("特效物品『" + item.BaseData.name + "』无法购买。");
                        continue;
                    }
                    uint price = KujiListFactory.Instance.ItemChoiceList[goods[i]];
                    if (price == 0) price = 1;
                    ...
                    if (gold >= price)
                    {
                        ...
                        Logger.LogItemGet(Logger.EventType.ItemNPCGet, this.Character.Name + "(" + this.Character.CharID + ")", item.BaseData.name + "(" + item.ItemID + ")",
                            string.Format("ChoiceShopBuy Count:{0}", item.Stack), false);
                        AddItem(item, true);
                    }
                }
            }
        ...
        public void OnVShopBuy(Packets.Client.CSMG_VSHOP_BUY p)
        {
            if (!vshopClosed)
            {
                ...
                if (count == 0 && items.Length > 0)
                {
                    ...
                    netIO.Disconnect();
                    Logger log = new Logger("被封玩家.txt");
                    string logtext = "\r\nECOShop交易封号：" + Character.Name;
        ...
        public void OnNPCShopSell(Packets.Client.CSMG_NPC_SHOP_SELL p)
        {
            ...
            if (this.currentShop != null)            
            {
                ...
                for (int i = 0; i < goods.Length; i++)
                {
                    ...
                    uint price = 0;
                    if (itemDroped.BaseData.itemType == ItemType.ARROW || itemDroped.BaseData.itemType == ItemType.BULLET||itemDroped.BaseData.itemType == ItemType.THROW)
                    {
                        price = 0;
                    }
                    else
        ...
        public void OnNPCEventStart(Packets.Client.CSMG_NPC_EVENT_START p)
        {
            ...
            {
                if (this.tradingTarget != null || trading || this.chara.Buff.憑依準備)
                {
                ...
                //if (p.EventID < 20000000 || p.EventID >= 0xF0000000)不知道为啥要限制编号
                if (true)
                {
