...
namespace SagaMap.Skill.SkillDefinations.Bard
{
    ...
    public class MAGINTDEXUPCircle : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            //註冊施放者的Buff
            DefaultBuff skill = new DefaultBuff(args.skill, sActor, "MAGINTDEXUPCircle_Caster", 15000 + 1000 * level);
        ...
        private class Activator : MultiRunTask
        {
                ...
                try
                {
                    if (count < countMax)
                    {
                        ...
                        foreach (Actor act in actors)
                        {
                            ...
                            {
                                if (!act.Status.Additions.ContainsKey("MAGINTDEXUPCircle"))
                                {
                                    DefaultBuff skill2 = new DefaultBuff(skill.skill, act, "MAGINTDEXUPCircle", lifeTime - count * period);
            ...
            void StartEventHandler(Actor actor, DefaultBuff skill)
            {
                ...
                short[] DEX = { 8, 11, 14, 17, 20 };
                short[] INT = { 8, 11, 14, 17, 20 };
                short[] MAG = { 8, 11, 14, 17, 20 };
