...
namespace SagaMap.Skill.SkillDefinations.BladeMaster
{
    public class Sinkuha : ISkill
    {
        ...
        public int TryCast(ActorPC pc, Actor dActor, SkillArg args)
        {
            if (CheckPossible(pc))
            {
                 SagaDB.Item.ItemType[] its = { ItemType.SWORD };
                 if (SkillHandler.Instance.CheckValidAttackTarget(pc, dActor))
                 {
                     return 0;
                 }
                 //else if (SkillHandler.Instance.CheckWeapon(pc, its)) return 0;
                 else
                 {
                     return -14;
                 }
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            if (CheckPossible(sActor))
            {
                ...
                if (level == 6)
                {
                    ...
                    actors.Add(dActor);
                SkillHandler.Instance.PhysicalAttack(sActor, actors, args, SagaLib.Elements.Neutral, factor);
