...
namespace SagaMap.Skill.SkillDefinations
{
    public class S25005 : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            ActorSkill actor = new ActorSkill(SagaDB.Skill.SkillFactory.Instance.GetSkill(31136,1), sActor)
            {
                Name = "丰饶之土",
                MapID = sActor.MapID,
                X = pos[0],
                    ...
                    if (count < countMax && acount < acountMax)//如果循环次数未到达临界，或者攻击次数未到达临界
                    {
                        ...
                        foreach (Actor i in actors)//遍历目标，不需要检查合法性，mob、玩家一起处理。
                        {
                            if (i.type == ActorType.PC || i.type == ActorType.MOB)//检查目标
                            {
                                ...
                                {
                                    OtherAddition skill = new OtherAddition(null, i, "丰饶之土", 20000);
                                    skill.OnAdditionStart += (x, e) =>
                                    {
                                        SkillHandler.Instance.ShowEffectOnActor(i, 4116);
                                        if (i.type == ActorType.PC)
                                            Network.Client.MapClient.FromActorPC((ActorPC)i).SendSystemMessage("获得『丰饶之土』，不会被击退、拉拽效果影响！");
                                    };
                                    skill.OnAdditionEnd += (x, e) =>
                                    {
                                        if (i.type == ActorType.PC)
                                            Network.Client.MapClient.FromActorPC((ActorPC)i).SendSystemMessage("『丰饶之土』效果结束了！");
                                    };
                                    SkillHandler.ApplyAddition(i, skill);
                                    //SagaMap.Manager.ScriptManager.Instance.Timers.ContainsKey("踩圈活动");
