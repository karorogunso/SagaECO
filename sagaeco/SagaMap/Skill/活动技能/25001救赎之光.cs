...
namespace SagaMap.Skill.SkillDefinations
{
    public class S25001 : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            ActorSkill actor = new ActorSkill(SagaDB.Skill.SkillFactory.Instance.GetSkill(31111,1), sActor)
            {
                Name = "救赎之光",
                MapID = sActor.MapID,
                X = pos[0],
        ...
        class 旋涡 : MultiRunTask
        {
            ...
            public override void CallBack()
            {
                try
                {
                    ...
                    {
                        count++;
                        List<Actor> actors = map.GetActorsArea(actor, 200, false);//获取技能周围的目标
                        bool target = false;
                        foreach (Actor i in actors)//遍历目标，不需要检查合法性，mob、玩家一起处理。
                        {
                            ...
                            {
                                target = true;
                                if (i.Status.Additions.ContainsKey("Silence")) SkillHandler.RemoveAddition(i, "Silence");
                                if (i.Status.Additions.ContainsKey("Stun")) SkillHandler.RemoveAddition(i, "Stun");
                                i.TInt["SPEEDDOWN"] = 0;
                                i.Buff.SpeedDown = false;
                                if (i.Status.Additions.ContainsKey("鈍足")) SkillHandler.RemoveAddition(i, "鈍足");
                                ...
                                if (!i.Status.Additions.ContainsKey("HolyVolition"))
                                {
                                    DefaultBuff skill = new DefaultBuff(null, i, "HolyVolition", 10000);
