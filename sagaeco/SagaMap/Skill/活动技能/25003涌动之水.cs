...
namespace SagaMap.Skill.SkillDefinations
{
    public class S25003 : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            ActorSkill actor = new ActorSkill(SagaDB.Skill.SkillFactory.Instance.GetSkill(31135,1), sActor)
            {
                Name = "涌动之水",
                MapID = sActor.MapID,
                X = pos[0],
                ...
                try
                {
                    ...
                    if (count < countMax && acount < acountMax)//如果循环次数未到达临界，或者攻击次数未到达临界
                    {
                        ...
                        foreach (Actor i in actors)//遍历目标，不需要检查合法性，mob、玩家一起处理。
                        {
                            ...
                            {
                                target = true;
                                if (!i.Status.Additions.ContainsKey("涌动之水"))
                                {
                                    OtherAddition skill = new OtherAddition(null, i, "涌动之水", 30000);
                                    skill.OnAdditionStart += (x, e) =>
                                    {
                                        SkillHandler.Instance.ShowEffectOnActor(i, 4115);
                                        if (i.type == ActorType.PC)
                                            Network.Client.MapClient.FromActorPC((ActorPC)i).SendSystemMessage("获得『涌动之水』，所有技能的冷却时间减半了！");
                                    };
                                    skill.OnAdditionEnd += (x, e) =>
                                    {
                                        if (i.type == ActorType.PC)
                                            Network.Client.MapClient.FromActorPC((ActorPC)i).SendSystemMessage("『涌动之水』效果结束了！");
                                    };
