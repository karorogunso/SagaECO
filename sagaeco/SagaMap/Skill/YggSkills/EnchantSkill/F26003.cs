    ...
    public class S26003:ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26003)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26003", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26003ElementUP"))
                    buff.Variable.Remove("Enchant26003ElementUP");
                buff.Variable.Add("Enchant26003ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26003CustomvalueUP"))
                    buff.Variable.Remove("Enchant26003CustomvalueUP");
                buff.Variable.Add("Enchant26003CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26003ElementUP"];
                int Customvalue = buff.Variable["Enchant26003CustomvalueUP"];
