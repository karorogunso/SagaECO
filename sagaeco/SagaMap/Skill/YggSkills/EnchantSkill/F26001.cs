    ...
    public class S26001:ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26001)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26001", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26001ElementUP"))
                    buff.Variable.Remove("Enchant26001ElementUP");
                buff.Variable.Add("Enchant26001ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26001CustomvalueUP"))
                    buff.Variable.Remove("Enchant26001CustomvalueUP");
                buff.Variable.Add("Enchant26001CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26001ElementUP"];
                int Customvalue = buff.Variable["Enchant26001CustomvalueUP"];
