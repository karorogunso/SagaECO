    ...
    public class S26002:ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26002)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26002", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26002ElementUP"))
                    buff.Variable.Remove("Enchant26002ElementUP");
                buff.Variable.Add("Enchant26002ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26002CustomvalueUP"))
                    buff.Variable.Remove("Enchant26002CustomvalueUP");
                buff.Variable.Add("Enchant26002CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26002ElementUP"];
                int Customvalue = buff.Variable["Enchant26002CustomvalueUP"];
