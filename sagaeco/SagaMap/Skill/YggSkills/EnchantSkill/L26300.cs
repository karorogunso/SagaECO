    ...
    public class S26300 : ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26300)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26300", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26300ElementUP"))
                    buff.Variable.Remove("Enchant26300ElementUP");
                buff.Variable.Add("Enchant26300ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26300CustomvalueUP"))
                    buff.Variable.Remove("Enchant26300CustomvalueUP");
                buff.Variable.Add("Enchant26300CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26300ElementUP"];
                int Customvalue = buff.Variable["Enchant26300CustomvalueUP"];
