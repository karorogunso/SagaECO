    ...
    public class S26201:ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26201)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26201", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26201ElementUP"))
                    buff.Variable.Remove("Enchant26201ElementUP");
                buff.Variable.Add("Enchant26201ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26201CustomvalueUP"))
                    buff.Variable.Remove("Enchant26201CustomvalueUP");
                buff.Variable.Add("Enchant26201CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26201ElementUP"];
                int Customvalue = buff.Variable["Enchant26201CustomvalueUP"];
