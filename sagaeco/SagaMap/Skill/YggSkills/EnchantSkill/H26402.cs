    ...
    public class S26402:ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26402)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26402", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26402ElementUP"))
                    buff.Variable.Remove("Enchant26402ElementUP");
                buff.Variable.Add("Enchant26402ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26402CustomvalueUP"))
                    buff.Variable.Remove("Enchant26402CustomvalueUP");
                buff.Variable.Add("Enchant26402CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26402ElementUP"];
                int Customvalue = buff.Variable["Enchant26402CustomvalueUP"];
