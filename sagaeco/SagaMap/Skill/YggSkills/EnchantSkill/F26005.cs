    ...
    public class S26005:ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26005)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26005", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26005ElementUP"))
                    buff.Variable.Remove("Enchant26005ElementUP");
                buff.Variable.Add("Enchant26005ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26005CustomvalueUP"))
                    buff.Variable.Remove("Enchant26005CustomvalueUP");
                buff.Variable.Add("Enchant26005CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26005ElementUP"];
                int Customvalue = buff.Variable["Enchant26005CustomvalueUP"];
