    ...
    public class S26200 : ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26200)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26200", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26200ElementUP"))
                    buff.Variable.Remove("Enchant26200ElementUP");
                buff.Variable.Add("Enchant26200ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26200CustomvalueUP"))
                    buff.Variable.Remove("Enchant26200CustomvalueUP");
                buff.Variable.Add("Enchant26200CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26200ElementUP"];
                int Customvalue = buff.Variable["Enchant26200CustomvalueUP"];
