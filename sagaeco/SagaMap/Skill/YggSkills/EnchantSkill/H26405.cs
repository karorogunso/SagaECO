    ...
    public class S26405:ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26405)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26405", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26405ElementUP"))
                    buff.Variable.Remove("Enchant26405ElementUP");
                buff.Variable.Add("Enchant26405ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26405CustomvalueUP"))
                    buff.Variable.Remove("Enchant26405CustomvalueUP");
                buff.Variable.Add("Enchant26405CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26405ElementUP"];
                int Customvalue = buff.Variable["Enchant26405CustomvalueUP"];
