    ...
    public class S26501:ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26501)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26501", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26501ElementUP"))
                    buff.Variable.Remove("Enchant26501ElementUP");
                buff.Variable.Add("Enchant26501ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26501CustomvalueUP"))
                    buff.Variable.Remove("Enchant26501CustomvalueUP");
                buff.Variable.Add("Enchant26501CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26501ElementUP"];
                int Customvalue = buff.Variable["Enchant26501CustomvalueUP"];
