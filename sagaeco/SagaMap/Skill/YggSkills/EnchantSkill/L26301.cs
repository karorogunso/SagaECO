    ...
    public class S26301:ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26301)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26301", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26301ElementUP"))
                    buff.Variable.Remove("Enchant26301ElementUP");
                buff.Variable.Add("Enchant26301ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26301CustomvalueUP"))
                    buff.Variable.Remove("Enchant26301CustomvalueUP");
                buff.Variable.Add("Enchant26301CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26301ElementUP"];
                int Customvalue = buff.Variable["Enchant26301CustomvalueUP"];
