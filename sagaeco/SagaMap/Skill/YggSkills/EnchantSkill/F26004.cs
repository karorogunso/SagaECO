    ...
    public class S26004:ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26004)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26004", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26004ElementUP"))
                    buff.Variable.Remove("Enchant26004ElementUP");
                buff.Variable.Add("Enchant26004ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26004CustomvalueUP"))
                    buff.Variable.Remove("Enchant26004CustomvalueUP");
                buff.Variable.Add("Enchant26004CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26004ElementUP"];
                int Customvalue = buff.Variable["Enchant26004CustomvalueUP"];
