    ...
    public class S26502:ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26502)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26502", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26502ElementUP"))
                    buff.Variable.Remove("Enchant26502ElementUP");
                buff.Variable.Add("Enchant26502ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26502CustomvalueUP"))
                    buff.Variable.Remove("Enchant26502CustomvalueUP");
                buff.Variable.Add("Enchant26502CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26502ElementUP"];
                int Customvalue = buff.Variable["Enchant26502CustomvalueUP"];
