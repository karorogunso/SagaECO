    ...
    public class S26204:ISkill
    {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                for (int i = 0; i < enums.Count; i++)
                {
                    if (pc.Inventory.Equipments.ContainsKey(enums[i]))
                    {
                        ...
                        if (item.BaseData.passiveSkill == 26204)
                        {
                            active = true;
                            amount += item.Refine;
                        }
                ...
                DefaultPassiveSkill skill = new DefaultPassiveSkill(args.skill, sActor, "Enchant26204", active, amount);
        ...
        void StartEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                ...
                if (buff.Variable.ContainsKey("Enchant26204ElementUP"))
                    buff.Variable.Remove("Enchant26204ElementUP");
                buff.Variable.Add("Enchant26204ElementUP", Elementvalue);

                if (buff.Variable.ContainsKey("Enchant26204CustomvalueUP"))
                    buff.Variable.Remove("Enchant26204CustomvalueUP");
                buff.Variable.Add("Enchant26204CustomvalueUP", Customvalue);
        ...
        void EndEventHandler(Actor actor, DefaultPassiveSkill buff)
        {
            if (actor.type == ActorType.PC)
            {
                int Elementvalue = buff.Variable["Enchant26204ElementUP"];
                int Customvalue = buff.Variable["Enchant26204CustomvalueUP"];
