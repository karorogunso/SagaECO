...
namespace SagaMap.Skill.SkillDefinations
{
    public class S13203 : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            /*OtherAddition skill = new OtherAddition(args.skill, sActor, "生命之重奏曲", 30000);
        ...
        private class Activator : MultiRunTask
        {
            ...
            public override void CallBack()
            {
                try
                {
                    ...
                    if (count < countMax && caster.SP >= 45)
                    {
                        ...
                        if (count % 10 == 0)
                        {
                            ...
                            foreach (Actor act in actors)
                            {
                                if (act.type == ActorType.PC)
                                {
                                    ...
                                    {
                                        if (!target.Status.Additions.ContainsKey("生命之重奏曲Buff"))
                                        {
                                            OtherAddition skill2 = new OtherAddition(null, target, "生命之重奏曲Buff", 2200);
                                            skill2.OnAdditionStart += (s, e) =>
                                            {
                                                act.TInt["生命之重奏曲Rate"] = val;
                                                act.TInt["生命之重奏曲HealRate"] = hprate;
                                                SkillHandler.Instance.ShowEffectOnActor(target, 4270);
                                            };
                                            skill2.OnAdditionEnd += (s, e) =>
                                            {
                                                act.TInt["生命之重奏曲Rate"] = 0;
                                                act.TInt["生命之重奏曲HealRate"] = 0;
                                            };
                                            SkillHandler.ApplyAddition(target, skill2);
                                        }
                                        else
                                        {
                                            Addition skill2 = target.Status.Additions["生命之重奏曲Buff"];
