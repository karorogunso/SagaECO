...
namespace SagaMap.Skill.SkillDefinations
{
    public class S13201 : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            /*OtherAddition skill = new OtherAddition(args.skill, sActor, "勇战之进行曲", 30000);
        ...
        private class Activator : MultiRunTask
        {
            ...
            public override void CallBack()
            {
                try
                {
                    ...
                    if (count < countMax && caster.SP >= 25)
                    {
                        ...
                        if (count % 10 == 0)
                        {
                            ...
                            foreach (Actor act in actors)
                            {
                                if (act.type == ActorType.PC)
                                {
                                    ...
                                    {
                                        if (!target.Status.Additions.ContainsKey("勇战之进行曲Buff"))
                                        {
                                            OtherAddition skill2 = new OtherAddition(null, target, "勇战之进行曲Buff", 2200);
                                            skill2.OnAdditionStart += (s, e) =>
                                            {
                                                act.TInt["勇战之进行曲Value"] = val;
                                                SkillHandler.Instance.ShowEffectOnActor(target, 4266);
                                            };
                                            skill2.OnAdditionEnd += (s, e) =>
                                            {
                                                act.TInt["勇战之进行曲Value"] = 0;
                                            };
                                            SkillHandler.ApplyAddition(target, skill2);
                                        }
                                        else
                                        {
                                            Addition skill2 = target.Status.Additions["勇战之进行曲Buff"];
