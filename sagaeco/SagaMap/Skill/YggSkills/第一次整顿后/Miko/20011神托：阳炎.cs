...
namespace SagaMap.Skill.SkillDefinations
{
    class S20011 : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            {
                SkillHandler.Instance.ShowEffectOnActor(sActor, 5105);
                OtherAddition buff = new OtherAddition(null, item, "神托：阳炎", lifetime);
                buff.OnAdditionStart += (s, e) =>
                {
                    item.Buff.ShieldFire = true;
                    item.TInt["火属性伤害降低"] = reduce;
                    map.SendEventToAllActorsWhoCanSeeActor(Map.EVENT_TYPE.BUFF_CHANGE, null, item, true);
                    SkillHandler.SendSystemMessage(item, "受到来自 " + sActor.Name + " 的神托：阳炎效果，受到火属性伤害减少。");
                };
                buff.OnAdditionEnd += (s, e) =>
                {
                    item.Buff.ShieldFire = false;
                    item.TInt["火属性伤害降低"] = 0;
                    map.SendEventToAllActorsWhoCanSeeActor(Map.EVENT_TYPE.BUFF_CHANGE, null, item, true);
                    SkillHandler.SendSystemMessage(item, "神托：阳炎效果效果消失了。");
                };
