...
namespace SagaMap.Skill.SkillDefinations
{
    public class S18503 : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            SkillHandler.Instance.PhysicalAttack(sActor, dactors, args, Elements.Neutral, factor);

            sActor.EP += 500;
            if (sActor.EP > sActor.MaxEP) sActor.EP = sActor.MaxEP;
            map.SendEventToAllActorsWhoCanSeeActor(Map.EVENT_TYPE.HPMPSP_UPDATE, null, sActor, true);

            SkillHandler.Instance.SetNextComboSkill(sActor, 18503);
        }
        #endregion
    }
}