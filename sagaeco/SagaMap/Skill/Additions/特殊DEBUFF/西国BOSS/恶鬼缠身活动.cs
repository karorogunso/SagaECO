...
{
    public class 恶鬼缠身活动 : MultiRunTask
    {
        Actor caster;
        Actor dActor;
        Map map;
        ...
        public 恶鬼缠身活动(Actor caster, Actor dactor, int damage = 550, int period = 900)
        {
            this.damage = damage;
            this.period = period;
            this.dueTime = 3000;
        ...
        public override void CallBack()
        {
            try
            {
                ...
                {
                    List<Actor> actors = map.GetActorsArea(dActor, 450, false);
                    foreach (var item in actors)
                    {
                        if (SkillHandler.Instance.CheckValidAttackTarget(caster, item))
                        {
                            if (!item.Tasks.ContainsKey("恶鬼缠身活动"))
                            {
                                恶鬼缠身活动 skill = new 恶鬼缠身活动(caster, item);
                                item.Tasks.Add("恶鬼缠身活动", skill);
                                skill.Activate();
                            }
                        }
                    }
                    SkillHandler.Instance.CauseDamage(caster, dActor, damage,true);
                    SkillHandler.Instance.ShowVessel(dActor, damage);
                    count++;
                }
                else
                {
                    dActor.Tasks.Remove("恶鬼缠身活动");
                    this.Deactivate();
