...
namespace SagaMap.Skill.Additions.Global
{
    public class DefaultBuff : Addition
    {
        ...
        public delegate void UpdateEventHandler2(Actor sActor, Actor dActor, DefaultBuff skill,SkillArg arg, int damage);
        ...
        public DefaultBuff(Actor actor, string name, int lifetime)
          : this(null, actor, name, lifetime, lifetime)
        {
        ...
        public DefaultBuff(SagaDB.Skill.Skill skill, Actor actor, Actor dActor, string name, int lifetime, int period, int damage)
            : this(skill,actor,dActor,name,lifetime,period,damage,null)
        {

        }
        public DefaultBuff(SagaDB.Skill.Skill skill, Actor sActor, Actor dActor, string name, int lifetime, int period, int damage,SkillArg arg)
        {
            this.Name = name;
            this.skill = skill;
            this.sactor = sActor;
        ...
        public override void AdditionStart()
        {
            ...
            {
                this.endTime = DateTime.Now + new TimeSpan(0, 0, 0, 0, lifeTime);// lifeTime / 60000, 0, 0, lifeTime);// (lifeTime / 1000) % 60);
                InitTimer(period, 0);
                TimerStart();
            }
