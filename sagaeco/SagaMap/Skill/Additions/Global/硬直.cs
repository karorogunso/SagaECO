...
namespace SagaMap.Skill.Additions.Global
{
    public class 硬直 : DefaultBuff 
    {
            ...
            : base(skill, actor, "硬直", lifetime)
        {
            ...
            {
                if (!actor.Status.Additions.ContainsKey("BOSS硬直免疫"))
                {
                    DefaultBuff BOSS硬直免疫 = new DefaultBuff(skill, actor, "BOSS硬直免疫", 30000);
                    SkillHandler.ApplyAddition(actor, BOSS硬直免疫);
                }
                else
                    this.Enabled = false;
            }
