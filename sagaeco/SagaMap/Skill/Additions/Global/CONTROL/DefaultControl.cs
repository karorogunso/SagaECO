...
{
    public class DefaultControl : Addition
    {
        public Actor sactor;
        ...
        public delegate void StartEventHandler(Actor actor, DefaultControl skill);
        public delegate void EndEventHandler(Actor actor, DefaultControl skill);
        public delegate void UpdateEventHandler(Actor actor, DefaultControl skill);
        public delegate void UpdateEventHandler2(Actor sActor, Actor dActor, DefaultControl skill,SkillArg arg, int damage);
        ...
        public DefaultControl(SagaDB.Skill.Skill skill, Actor actor, string name, int lifetime)
            : this(skill, actor, name, lifetime, lifetime)
        {

        }

        public DefaultControl(SagaDB.Skill.Skill skill, Actor actor, string name, int lifetime, int period)
            : this(skill, null, actor, name, lifetime, period, 0,null)
        {

        }
        public DefaultControl(SagaDB.Skill.Skill skill, Actor actor, Actor dActor, string name, int lifetime, int period, int damage)
            : this(skill,actor,dActor,name,lifetime,period,damage,null)
        {

        }
        public DefaultControl(SagaDB.Skill.Skill skill, Actor sActor, Actor dActor, string name, int lifetime, int period, int damage,SkillArg arg)
        {
            this.Name = name;
            this.skill = skill;
            this.sactor = sActor;
            ...
            this.damage = damage;
            this.args = arg;
            this.MyType = AdditionType.Control;
        }
