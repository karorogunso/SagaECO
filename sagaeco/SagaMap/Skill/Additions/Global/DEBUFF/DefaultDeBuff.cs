...
{
    public class DefaultDeBuff : Addition
    {
        public Actor sactor;
        ...
        public delegate void StartEventHandler(Actor actor, DefaultDeBuff skill);
        public delegate void EndEventHandler(Actor actor, DefaultDeBuff skill);
        public delegate void UpdateEventHandler(Actor actor, DefaultDeBuff skill);
        public delegate void UpdateEventHandler2(Actor sActor, Actor dActor, DefaultDeBuff skill,SkillArg arg, int damage);
        ...
        public DefaultDeBuff(SagaDB.Skill.Skill skill, Actor actor, string name, int lifetime)
            : this(skill, actor, name, lifetime, lifetime)
        {

        }

        public DefaultDeBuff(SagaDB.Skill.Skill skill, Actor actor, string name, int lifetime, int period)
            : this(skill, null, actor, name, lifetime, period, 0,null)
        {

        }
        public DefaultDeBuff(SagaDB.Skill.Skill skill, Actor actor, Actor dActor, string name, int lifetime, int period, int damage)
            : this(skill,actor,dActor,name,lifetime,period,damage,null)
        {

        }
        public DefaultDeBuff(SagaDB.Skill.Skill skill, Actor sActor, Actor dActor, string name, int lifetime, int period, int damage,SkillArg arg)
        {
            this.Name = name;
            this.skill = skill;
            this.sactor = sActor;
            ...
            this.damage = damage;
            this.args = arg;
            this.MyType = AdditionType.Debuff;
        }
