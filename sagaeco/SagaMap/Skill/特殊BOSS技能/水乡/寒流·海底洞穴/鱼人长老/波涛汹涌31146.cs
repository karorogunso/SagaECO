    ...
    public class S31146 : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            foreach (var item in actors)
            {
                if(SkillHandler.Instance.CheckValidAttackTarget(sActor,item))
                {
                    ...
                    {
                        short[] pos;
                        pos = map.GetRandomPosAroundPos(item.X, item.Y, 200);
        ...
        class 魔法阵 : MultiRunTask //定一个法阵类，用来给每个玩家实例这个法阵。继承MultiRunTask
        {
            ...
            public 魔法阵(Actor sActor, ActorSkill actorSkill)//传入2个角色，一个是攻击者sActor，一个是目标dActor
            {
                ...
                dueTime = SagaLib.Global.Random.Next(2000,2500);//设置启动延迟为2秒
            ...
            public override void CallBack()//启动延迟2秒后，执行这个函数
            {
                try//如果执行try里面的内容发生了错误，则立刻跳到下面的catch{ }中
                {
                    ...
                    SkillHandler.Instance.ShowEffect(map, sActor, x, y, 5031);
                    ...
                    {
                        SkillHandler.Instance.DoDamage(false, sActor, item, null, SkillHandler.DefType.MDef, Elements.Water, 50, 5f);
                        if(item.Status.Additions.ContainsKey("波涛汹涌"))
                        {
                            Freeze freeze = new Freeze(null, item, 3000, 100);
                            SkillHandler.ApplyAddition(item, freeze);
                        }
                        else
                        {
                            OtherAddition skill = new OtherAddition(null, item, "波涛汹涌", 1000);
