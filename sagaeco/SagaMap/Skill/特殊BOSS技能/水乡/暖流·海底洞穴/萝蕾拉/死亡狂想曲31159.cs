                    ...
                    if (count < countMax && dactor != null)
                    {
                        ...
                        foreach (var item in targets)
                        {
                            if (SkillHandler.Instance.CheckValidAttackTarget(caster, item))
                            {
                                ...
                                pos[0] = item.X;
                                pos[1] = item.Y;
                                SkillHandler.Instance.ShowEffect(map, caster, SagaLib.Global.PosX16to8(pos[0], map.Width), SagaLib.Global.PosY16to8(pos[1], map.Height), 7014);
        ...
        private class Activator2 : MultiRunTask
        {
            ...
            public override void CallBack()
            {
                ...
                try
                {
                    ...
                    List<Actor> actors = map.GetRoundAreaActors(SagaLib.Global.PosX8to16(s.x, map.Width), SagaLib.Global.PosY8to16(s.y, map.Height), 200);
                    ...
                    foreach (Actor i in actors)
                    {
                        ...
                        {
                            SkillHandler.Instance.DoDamage(false, caster, i, null, SkillHandler.DefType.MDef, Elements.Fire, 50, 4f);
                            SkillHandler.Instance.ShowEffect(Manager.MapManager.Instance.GetMap(i.MapID), i, 5261);
                        }
                    }
                    this.Deactivate();
                }
