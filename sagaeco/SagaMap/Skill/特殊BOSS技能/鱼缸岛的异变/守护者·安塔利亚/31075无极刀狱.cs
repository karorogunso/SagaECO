                ...
                try
                {
                    if (count == 10)
                    {
                        if (caster.type == ActorType.MOB)
                            SkillHandler.Instance.ActorSpeak(caster, "为了守护这一切。");
                    }
                    if (count < countMax)
                    {
            ...
            public override void CallBack()
            {
                ...
                try
                {
                    if (count < countMax)
                    {
                        ...
                        short[] pos1 = map.GetRandomPosAroundPos(caster.X, caster.Y, 500);
            ...
            public override void CallBack()
            {
                ...
                try
                {
                    if (count < countMax && dactor != null)
                    {
                        ...
                        actors = map.GetActorsArea(actor, 150, false);
                        ...
                        if (affected.Count > 0 || dactor != null)
                        {
                            foreach (Actor j in affected)
                            {
                                ...
                                {
                                    int damage = SkillHandler.Instance.CalcDamage(true, caster, j, skill, SkillHandler.DefType.IgnoreAll, Elements.Neutral, 1, 1.2f);
