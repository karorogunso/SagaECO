...
namespace SagaMap.Skill.SkillDefinations
{
    public class S31079 : ISkill
    {
        ...
        {
            SkillHandler.Instance.ActorSpeak(sActor, "出来吧——！我的小弟们！这是给你们锻炼的好机会！！");
            Map map = Manager.MapManager.Instance.GetMap(sActor.MapID);
            byte x, y;
            int count = 3 - sActor.Slave.Count;
            if (count <= 3)
            {
        ...
        private class Activator : MultiRunTask
        {
            ...
            public override void CallBack()
            {
                try
                {
                    ActorMob mob = map.SpawnCustomMob(10560000, map.ID, 15185000, 0, 0, x, y, 0, 1, 0, 孤魂info((uint)(caster.MaxHP * 0.05f)), 孤魂ai(), null, 0)[0];
