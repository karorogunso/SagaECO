...
namespace SagaMap.Skill.SkillDefinations
{
    public class S30009 : ISkill
    {
        ...
        {
            ActorSkill actor = new ActorSkill(SagaDB.Skill.SkillFactory.Instance.GetSkill(30009, 1), sActor);
            byte X = 0;
            byte Y = 0;
            ...
            public override void CallBack()
            {
                ...
                try
                {
                    ...
                    if (path.Count > 1)
                    {
                        ...
                        actors = map.GetActorsArea(Boll, 300, false);
                        foreach (var a in actors)
                        {
                            if(SkillHandler.Instance.CheckValidAttackTarget(sActor,a))
                            {
                                short[] pos2 = new short[2] { Boll.X, Boll.Y };
                                map.MoveActor(Map.MOVE_TYPE.START, a, pos2, 1000, 1000, true, MoveType.QUICKEN);
                                int damaga = SkillHandler.Instance.CalcDamage(false, sActor, a, arg, SkillHandler.DefType.MDef, Elements.Wind, 50, 1f);
                    ...
                    else
                    {
                       ...
                       map.SendEffect(Boll, SagaLib.Global.PosX16to8(Boll.X, map.Width), SagaLib.Global.PosY16to8(Boll.Y, map.Height), 4029);
                        //Skill.SkillHandler.Instance.ShowEffect(map, dActor, 4029);
                        foreach (var a in actors)
                        {
                            if (SkillHandler.Instance.CheckValidAttackTarget(sActor, a))
                            {
                                short[] pos2 = new short[2] { Boll.X, Boll.Y };
                                map.MoveActor(Map.MOVE_TYPE.START, a, pos2, 1000, 1000, true, MoveType.QUICKEN);
                                if (!SkillHandler.Instance.isBossMob(a))
                                    SkillHandler.Instance.PushBack(Boll, a, 8);
                                int damaga = SkillHandler.Instance.CalcDamage(false, sActor, a, arg, SkillHandler.DefType.MDef, Elements.Wind, 50, 7f);
                                SkillHandler.Instance.CauseDamage(sActor, a, damaga);
                                SkillHandler.Instance.ShowVessel(a, damaga);
                                Stun s = new Stun(arg.skill, a, 3000);
