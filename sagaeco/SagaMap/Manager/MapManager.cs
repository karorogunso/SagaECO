...
namespace SagaMap.Manager
{
    ...
    {
        private IConcurrentDictionary<uint, Map> maps;
        private IConcurrentDictionary<uint, MapInfo> mapInfo;
        ...
        public MapManager()
        {   
            this.maps = new IConcurrentDictionary<uint, Map>();
            this.mapInfo = new IConcurrentDictionary<uint, MapInfo>();            
        }
        ...
        public IConcurrentDictionary<uint, Map> Maps
        {
            get
            {
                return this.maps;
        ...
        public IConcurrentDictionary<uint, MapInfo> MapInfos
        {
            set
            {
                this.mapInfo = value;
        ...
        public uint CreateMapInstance(ActorPC creator, uint template, uint exitMap, byte exitX, byte exitY, bool autoDispose, uint ResurrectionLimit, bool returnori)
        {
            ...
            {
                for (int i = (int)(template % 1000); i < 9999; i++)
                {
                    if (!this.maps.ContainsKey((uint)((template / 1000 * 10000) + i)))
                    {
                        newMap.ID = (uint)((template / 1000 * 10000) + i);
                        if (creator != null)
                            Logger.ShowInfo(newMap.ID.ToString() + "副本创建者：" + creator.Name);
                        else
                            Logger.ShowInfo(newMap.ID.ToString() + "副本创建者：获取失败");
                        break;
                    }
                }
            }
            newMap.DeleteTime = DateTime.Now + new TimeSpan(InstanceMapLifeHour, 0, 0);
            if (template == 70000000 || template == 75000000)
                newMap.DeleteTime = DateTime.Now + new TimeSpan(2400, 0, 0);
            ...
            if (!creator.InstanceMapIDs.Contains(newMap.ID))
                creator.InstanceMapIDs.Add(newMap.ID);
            return newMap.ID;
        }
