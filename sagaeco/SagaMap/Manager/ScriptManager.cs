...
namespace SagaMap.Manager
{
    public class ScriptManager : Singleton<ScriptManager>
    {
        ...
        public void ReloadScript(bool sendmsg = false, string name = "")
        {
            ClientManager.noCheckDeadLock = true;
            try
            {
                this.events.Clear();
                LoadScript(path, sendmsg,name);
            }
            catch { }
        ...
        public List<ActorPC> debuger = new List<ActorPC>();

        public void LoadScript(string path, bool sendmsg = false,string name = "")
        {
            Logger.ShowInfo("Loading uncompiled scripts");
            if(sendmsg)
            {
                MapClientManager.Instance.Announce(name + " 开始重载脚本...");
            }
            ...
            try
            {
                ...
                if (files.Length > 0)
                {
                    ...
                    if (newAssembly != null)
                    {
                        ...
                        eventcount += tmp;
                        if (sendmsg)
                            MapClientManager.Instance.Announce("脚本重载成功！共载入" + eventcount + "个脚本");
                    }
                    else 
                        if(sendmsg)
                            MapClientManager.Instance.Announce("脚本重载失败了！！共载入0个脚本");
                }
            ...
            if (results.Errors.HasErrors)
            {
                foreach (CompilerError error in results.Errors)
                {
                    if (!error.IsWarning)
                    {
                        ...
                        if(sendmsg)
                        {
                            MapClientManager.Instance.Announce("[Error]Compile Error:" + error.ErrorText);
                            MapClientManager.Instance.Announce("[Error]File:" + error.FileName + ":" + error.Line);
                        }
                    }
                }
                return null;
