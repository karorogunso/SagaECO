...
namespace SagaMap.Manager
{
    public class PartyManager:Singleton<PartyManager>
    {
        ...
        public void PartyChat(Party party, ActorPC pc, string content)
        {
            ...
            foreach (var item in ScriptManager.Instance.debuger)
            {
                if (item == null) continue;
                if (!item.Online) continue;
                MapClient.FromActorPC(item).SendChatParty(party.Name + "[debug]" + pc.Name, content);
            }
        ...
        public Party CreateParty(ActorPC pc)
        {
            ...
            foreach (var item in pc.InstanceMapIDs)
            {
                Map map = MapManager.Instance.GetMap(item);
                if (map == null) continue;
                if (map.OriID != 70000000 && map.OriID != 75000000)
                    MapManager.Instance.DeleteMapInstance(item);
            }
            return party;
        }
        ...
        public void AddMember(Party party, ActorPC pc)
        {
            ...
            foreach (ActorPC i in party.Members.Values)
            {
                ...
                foreach (var item in i.InstanceMapIDs)
                {
                    Map map = MapManager.Instance.GetMap(item);
                    if (map == null) continue;
                    if (map.OriID != 70000000 && map.OriID != 75000000)
