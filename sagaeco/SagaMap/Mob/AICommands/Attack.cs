    ...
    public class Attack : AICommand
    {
        ...
        private Actor CurrentTarget()
        {
            try
            {
                ...
                uint hate = 0;
                Actor tmp = null;
                uint[] ids = new uint[mob.Hate.Keys.Count + 2];
        ...
        private void CheckAggro()
        {
            ...
            foreach (KeyValuePair<uint, uint> id in mob.Mob.VisibleActors)
            {
                if (mob.map == null)
                    continue;
                Actor i = mob.map.GetActor(id.Key);
                if (i == null)
                    continue;
                ...
                if (mob.Mob.type != ActorType.PC && mob.Mob.type != ActorType.GOLEM&& !mob.Mob.Buff.ゾンビ && i.type != ActorType.PC && i.type != ActorType.PET && i.type != ActorType.PARTNER && i.type != ActorType.SHADOW && !(i.type == ActorType.MOB && isSlavaOfPc))
                    continue;
        ...
        private bool hasPlayerInSight()
        {
            if (mob.Mob.type == ActorType.PC)
                return true;
            foreach (KeyValuePair<uint,uint> item in mob.Mob.VisibleActors)
            {
                uint id = item.Key;
            ...
            for (uint j = 0; j < mob.Mob.VisibleActors.Count;j++)
            {
                uint id = mob.Mob.VisibleActors[j];
                Actor i = mob.map.GetActor(id);
                if (i == null)
                    continue;
                if (i.MapID != this.mob.map.ID)
        ...
        public void Update(object para)
        {
            try
            {
                ...
                List<uint> hateIDs = new List<uint>();
                foreach (var item in mob.Hate.Keys)
                    hateIDs.Add(item);
                bool CanInit = false;
                if (hateIDs.Count == 1)
                    if (hateIDs[0] == mob.Mob.ActorID)
                        mob.Hate.Remove(hateIDs[0]);
                if (this.mob.Hate.Count == 0  || CanInit)
                {
                    ...
                    if (DateTime.Now > this.mob.BackTimer.AddSeconds(3) && attacking && !mob.noreturn)//5秒后返回
                    {
                        ...
                        try
                        {
                            if (mob.Mob.Slave.Count != 0)
                            {
                                List<Actor> slavelist = new List<Actor>(mob.Mob.Slave);
                                foreach (Actor a in slavelist)
                                {
                                    a.e.OnDie();
                                }
                            }
                ...
                //if (mob.Mob.type == ActorType.GOLEM)
                {
                    ...
                    {
                        if (dest.Status != null)
                            if ((mob.Mode.Active || mob.Mob.Buff.ゾンビ) && (dest == null || dest == mob.Master))
                                CheckAggro();
                    }
                    else if (dest == null)
                    {
