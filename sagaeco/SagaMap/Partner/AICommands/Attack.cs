    ...
    public class Attack : AICommand
    {
        ...
        private Actor CurrentTarget()
        {
            try
            {
                ...
                if (partner.Owner.HP == 0 || partner.Owner.Buff.Dead)
                    id = 0;
        ...
        private void CheckAggro()
        {
            ...
            foreach (KeyValuePair<uint, uint> id in partnerai.Partner.VisibleActors)
            {
                Actor i = partnerai.map.GetActor(id.Key);
                if (i == null)
                    continue;
                ...
                if (!partnerai.Partner.Buff.ゾンビ && i.type != ActorType.PC && i.type != ActorType.PET && i.type != ActorType.SHADOW && !(i.type == ActorType.MOB && isSlavaOfPc))
                    continue;
        ...
        public void Update(object para)
        {
            ...
            if (attacktask == null)
                attacktask = new PartnerAttack(partnerai, dest);
            dest = CurrentTarget();
            if ((partnerai.Mode.Active || partnerai.Partner.Buff.ゾンビ) && (dest == null || dest == partnerai.Master))
            {
                CheckAggro();
            }
            if (dest == null)
