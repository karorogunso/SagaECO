...
namespace SagaMap
{
    public partial class Map
    {
        ...
        private ConcurrentDictionary<ulong, Actor> actorsByID;
        private ConcurrentDictionary<uint, ConcurrentDictionary<ulong, Actor>> actorsByRegion;
        ...
        private const uint ID_BORDER_EVENT = 60002;
        ...
        public Map(MapInfo info)
        {
            ...
            this.actorsByRegion = new ConcurrentDictionary<uint, ConcurrentDictionary<ulong, Actor>>();
        ...
        /// 取得当前地图的Actor列表
        /// </summary>        
        public ConcurrentDictionary<ulong, Actor> Actors { get { return actorsByID; } }
        ...
        public bool RegisterActor(Actor nActor)
        {
            ...
            if (newID != 0)
            {
                ...
                if (!actorsByRegion.TryGetValue(nActor.region, out region))
                {
                    region = new ConcurrentDictionary<ulong, Actor>();
                    actorsByRegion[nActor.region] = region;
                }
    ...
    pc.Mode == PlayerMode.KNIGHT_ROCK || pc.Mode == PlayerMode.KNIGHT_SOUTH || pc.Mode == PlayerMode.KNIGHT_WEST)
        ...
        public void DeleteActor(Actor dActor)
        {
            ...
            Actor obj;
            actorsByID.TryRemove(dActor.ActorID, out obj);

            ConcurrentDictionary<ulong, Actor> region;
            if (actorsByRegion.TryGetValue(dActor.region, out region))
            {
                Actor removed;
        ...
        public void MoveActor(MOVE_TYPE mType, Actor mActor, short[] pos, ushort dir, ushort speed, bool sendToSelf,MoveType moveType)
        {
            ...
            try
            {
                ...
                if (mActor.type == ActorType.PC)
                {
                    ...
                    Network.Client.MapClient.FromActorPC(pc).SendChangeStatus();
                ...
                if (!knockBack)
                {
                    for (short deltaY = -1; deltaY <= 1; deltaY++)
                    {
                        for (short deltaX = -1; deltaX <= 1; deltaX++)
                        {
                            ...
                            if (!actorsByRegion.TryGetValue(region, out actors)) continue;
                ...
                if (mActor.region != newRegion)
                {
                    ...
                    if (actorsByRegion.TryGetValue(mActor.region, out list))
                    {
                        Actor removed;
                    ...
                    if (!actorsByRegion.TryGetValue(newRegion, out list))
                    {
                        list = new ConcurrentDictionary<ulong, Actor>();
                        actorsByRegion[newRegion] = list;
                    }
        ...
        public int GetRegionPlayerCount(uint region)
        {
            ...
            if (this.actorsByRegion.TryGetValue(region, out actors))
            {
                ...
                {
                    if (actor.Value.type == ActorType.PC) count++;
                }
                return count;
            }
            else return 0;
        ...
        public bool ACanSeeB(Actor A, Actor B)
        {
            ...
            if (B.type == ActorType.FURNITURE)
            {
                if (System.Math.Abs(A.Y - ((ActorFurniture)B).Z) > A.sightRange) return false;
            }
            else
        ...
        public bool ACanSeeB(Actor A, float ax, float ay, Actor B)
        {
            ...
            if (B.type == ActorType.FURNITURE)
            {
                if (System.Math.Abs(ay - ((ActorFurniture)B).Z) > A.sightRange) return false;
            }
            else
        ...
        public bool ACanSeeB(Actor A, Actor B, float sightrange)
        {
            ...
            if (B.type == ActorType.FURNITURE)
            {
                if (System.Math.Abs(A.Y - ((ActorFurniture)B).Z) > sightrange) return false;
            }
            else
                        ...
                        try
                        {
                            ...
                            if (this.ACanSeeB(jActor, actor))
                            {
                                /*if (actor.type == ActorType.FURNITURE &&
                                    ItemFactory.Instance.GetItem(((ActorFurniture)actor).ItemID).BaseData.itemType != ItemType.FF_CASTLE &&
                                    id > 90001000)
                                {
                                    listAF.Add(actor);
                                }*/
                                if(actor.type == ActorType.FURNITURE &&
                                    ItemFactory.Instance.GetItem(((ActorFurniture)actor).ItemID).BaseData.itemType != ItemType.FF_CASTLE && (id >= 70000000 || id == 10054000))
                                {
                                    listAF.Add(actor);
                                }
                                else
        ...
        public void TeleportActor(Actor sActor, short x, short y)
        {
            ...
            Actor removed;
            actorsByRegion[sActor.region].TryRemove(sActor.ActorID, out removed);
        ...
        public void SendActorToMap(Actor mActor, uint mapid, short x, short y,bool possession)
        {
            ...
            if (mActor.type == ActorType.PC)
            {
                ...
                if (pc.PossesionedActors.Count > 0)
                    Skill.SkillHandler.Instance.PossessionCancel(pc, PossessionPosition.NONE);
                if (pc.PossessionTarget != 0 && !possession)
                    return;
                /*List<ActorPC> possessioned = pc.PossesionedActors;
                foreach (ActorPC i in possessioned)
