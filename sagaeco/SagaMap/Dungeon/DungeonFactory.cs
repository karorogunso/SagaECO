    ...
    public class DungeonFactory : Factory<DungeonFactory, Dungeon>
    {
        ...
        public Dungeon CreateDungeon(uint id, ActorPC creator, uint exitMap, byte exitX, byte exitY)
        {
            ...
            if (items.ContainsKey(id))
            {
                ...
                dungeon.Start.Map = MapManager.Instance.GetMap(MapManager.Instance.CreateMapInstance(creator, dungeon.StartMap, exitMap, exitX, exitY, false, 999, true));
                ...
                while (roomcount > 0 || floorcount > 0 || crosscount > 0 || endcount > 0)
                {
                    ...
                    nowMap.Map = MapManager.Instance.GetMap(MapManager.Instance.CreateMapInstance(creator, nowMap.ID, exitMap, exitX, exitY,false,999,true));
                    nowMap.Map.IsDungeon = true;
                    //nowMap.Map.returnori = true;
