...
using SagaDB.Quests;
//所在地圖:奧克魯尼亞西部平原(10022000) NPC基本信息:咖啡館店員(11000436) X:105 Y:136
namespace SagaScript.M10022000
{
    public class S11000436 : Event
    {
        public S11000436()
        {
            this.EventID = 11000436;
        ...
        void 埃米爾介紹書(ActorPC pc)
        {
            ...
            Say(pc, 11000436, 131, "那麼，要挑戰任務嗎?$R;", "咖啡館店員");
        ...
        void 任務種類詳細解說(ActorPC pc)
        {
            ...
            Say(pc, 11000436, 131, "任務的要求幾乎都很簡單喔!$R;" +
                                   "$R「咖啡館」除了賣糧食外，$R;" +
                                   "也會介紹一些工作給冒險者唷!$R;" +
                                   "$P久而久之，口碑越來越好了，$R;" +
                                   "所以在「阿高普路斯市」周圍，$R;" +
            ...
            while (selection != 5)
            {
                ...
                {
                    case 1:
                        Say(pc, 11000436, 131, "成功完成任務的話，$R;" +
                                               "可以得到相對應的經驗值和報酬。$R;" +
                                               "$R但托付的任務並不是很多，$R;" +
                                               "有時候會供過於求，不太平衡啊!$R;" +
                                               "$P所以工作是有次數限制的。$R;" +
                    ...
                    case 2:
                        Say(pc, 11000436, 131, "「擊退任務」就是要在指定的區域，$R;" +
                                               "抓到指定數量的魔物。$R;" +
                                               "$P例如：$R;" +
                                               "擊退「奧克魯尼亞東方平原」的$R;" +
                                               "5隻「皮露露」。$R;" +
                    ...
                    case 3:
                        Say(pc, 11000436, 131, "「收集任務」就是收集指定道具的任務唷!$R;" +
                                               "$P如果接到收集3個『杰利科』的任務。$R;" +
                                               "$R只要想盡辦法收集3個『杰利科』，$R;" +
                                               "就算任務完成了。$R;" +
                                               "$P收集完以後，$R;" +
                    ...
                    case 4:
                        Say(pc, 11000436, 131, "「搬運任務」是從委託人那裡取得道具，$R;" +
                                               "然後轉交給收件人的任務唷!$R;" +
                                               "$P例如：$R;" +
                                               "在「奧克魯尼亞東方平原」的$R;" +
                                               "「咖啡館店員」那裡$R;" +
        ...
        void 擊退皮露露(ActorPC pc)
        {
            ...
            Say(pc, 11000436, 131, "最近「阿高普路斯市」周圍，$R;" +
                                   "出現了很多「皮露露」，$R;" +
                                   "能不能擊退呢?$R;" +
                                   "$R「皮露露」是像布丁的天藍色魔物。$R;" +
                                   "$P在任務清單選擇任務後，$R;" +
