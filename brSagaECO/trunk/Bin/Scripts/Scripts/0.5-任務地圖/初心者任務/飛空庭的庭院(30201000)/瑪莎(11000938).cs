...
namespace SagaScript.M30201000
{
    public class S11000938 : Event
    {
        ...
        void 與瑪莎進行第一次對話(ActorPC pc)
        {
            ...
            Say(pc, 11000938, 131, "歡迎來到飛空庭唷!$R;" +
                                   "$P飛空庭是飛在空中的家和庭園喔!$R;" +
                                   "$R在ECO世界，$R;" +
                                   "您可以擁有自己的房子和庭園。$R;" +
                                   "$P飛空庭是由「唐卡」的工匠製作的。$R;" +
        ...
        void 與瑪莎進行第二次對話(ActorPC pc)
        {
            ...
            if (Beginner_01_mask.Test(Beginner_01.發現床底下的東西))
            {
                ...
                Say(pc, 11000938, 131, "不是經常失敗的，$R;" +
                                       "只是偶~~~爾吧!!$R;", "瑪莎");

                Say(pc, 11000938, 134, "嗚嗚…$R;" +
                                       "$R本來想以後自己拿到古董商店那裡的。$R;" +
                                       "$P現在去不去下一階段呢?$R;", "瑪莎");
            }
            ...
            switch (Select(pc, "去下一階段嗎?", "", "去下一階段", "再告訴我一次關於飛空庭的情報", "再休息一會兒"))
            {
                ...
                case 2:
                    Say(pc, 11000938, 131, "飛空庭是由「唐卡」的工匠製作的唷!$R;" +
                                           "$R收集部件有點困難呀…$R;" +
                                           "$P對了!$R;" +
                                           "$R有個村落裡有個工匠，找找看吧，$R;" +
                                           "他人挺好的，$R;" +
