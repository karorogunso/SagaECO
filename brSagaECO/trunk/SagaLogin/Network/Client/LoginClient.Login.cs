...
namespace SagaLogin.Network.Client
{
    public partial class LoginClient : SagaLib.Client
    {
        ...
        public void OnReconnect(Packets.Client.CSMG_RECONNECT p)
        {
        ...
        private void SendWelcomeMessages()
        {
            if (Configuration.Instance.Version >= SagaLib.Version.Saga18)
            {
                ...
                p.Type = Packets.Server.SSMG_CHAT_SYSTEM_MESSAGE.MessageType.YellowSystemMessage;
                p.Content = "------------------------------------------------------";
                this.netIO.SendPacket(p);
                foreach (string motd in Configuration.Instance.Motd)
                {
                    Packets.Server.SSMG_CHAT_SYSTEM_MESSAGE p1 = new Packets.Server.SSMG_CHAT_SYSTEM_MESSAGE();
                    p1.Type = Packets.Server.SSMG_CHAT_SYSTEM_MESSAGE.MessageType.YellowSystemMessage;
                    p1.Content = motd;
                    this.netIO.SendPacket(p1);
                }
                this.netIO.SendPacket(p);
