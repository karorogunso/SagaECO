...
namespace SagaDB.DEMIC
{
    public class ChipFactory : Factory<ChipFactory, Chip.BaseData>
    {
        ...
        protected override void ParseCSV(Chip.BaseData item, string[] paras)
        {
            ...
            item.hp = short.Parse(paras[11]);
            item.mp = short.Parse(paras[12]);
            item.sp = short.Parse(paras[13]);
            item.str = short.Parse(paras[14]);
            item.mag = short.Parse(paras[15]);
            item.vit = short.Parse(paras[16]);
            item.dex = short.Parse(paras[17]);
            item.agi = short.Parse(paras[18]);
            item.intel = short.Parse(paras[19]);
            item.skill1 = uint.Parse(paras[20]);
            item.skill2 = uint.Parse(paras[21]);
            item.skill3 = uint.Parse(paras[22]);
        }
    }
}
