...
namespace SagaDB.Item
{
    ...
    public class Inventory
    {
        ...
        private Dictionary<EnumEquipSlot, Item> parts = new Dictionary<EnumEquipSlot, Item>();
        ...
        public InventoryAddResult AddWareItem(WarehousePlace place, Item item)
        {
            try
            {
                ...
                if (query.Count() != 0 && item.Stackable)
                {
                    ...
                    {
                        ushort rest = (ushort)(oriItem.Stack - 999);
                        if (rest > 999)
                        {
                            Logger.ShowWarning("Adding too many item(" + item.BaseData.name + ":" + item.Stack + "), setting count to the maximal value(999)");
                            rest = 999;
                        }
                        oriItem.Stack = 999;
                        item.Stack = oriItem.Stack;
