...
namespace SagaMap.Network.Client
{
    public partial class MapClient : SagaLib.Client 
    {
        ...
        public static MapClient FromActorPC(ActorPC pc)
        {
            try
            {
                return ((ActorEventHandlers.PCEventHandler)pc.e).Client;
            }
            catch (Exception ex)
            {
                throw ex;
            ...
            try
            {
                ...
                if (golemLogout && this.chara.PossessionTarget == 0)
                {
                    ...
                    if (this.Character.Golem.BuyLimit > this.Character.Gold)
                        this.Character.Golem.BuyLimit = (long)this.Character.Gold;
        ...
        {
            MapManager.Instance.DisposeMapInstanceOnLogout(this.chara);
            foreach (KeyValuePair<string, MultiRunTask> i in chara.Tasks)
            {
                if (i.Value is Scripting.Timer)
                {
            ...
            this.chara.KillingMarkCounter = 0;
            this.chara.KillingMarkSoulUse = false;
            this.chara.Tasks.Clear();
            ...
            //this.chara.Status.elements_base.Clear();
            //this.chara.Status.elements_item.Clear();
            //this.chara.Status.elements_skill.Clear();
            //this.chara.Status.attackElements_base.Clear();
            //this.chara.Status.attackElements_item.Clear();
            //this.chara.Status.attackElements_skill.Clear();
            this.chara.Pet = null;
