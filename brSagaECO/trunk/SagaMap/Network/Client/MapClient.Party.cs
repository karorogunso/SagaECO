...
namespace SagaMap.Network.Client
{
    public partial class MapClient
    {
        ...
        public void OnPartyName(Packets.Client.CSMG_PARTY_NAME p)
        {
            ...
            if (string.IsNullOrEmpty(p.Name) || string.IsNullOrWhiteSpace(p.Name) || p.Name == "")
                return;
        ...
        public void OnPartyKick(Packets.Client.CSMG_PARTY_KICK p)
        {
            ...
            {
                PartyManager.Instance.DeleteMember(this.Character.Party, p.CharID, SagaMap.Packets.Server.SSMG_PARTY_DELETE.Result.KICKED);
            }
            else
            {
                p1.Result = -1;
        ...
        int CheckPartyInvition(MapClient client)
        {
            if (client == null)
