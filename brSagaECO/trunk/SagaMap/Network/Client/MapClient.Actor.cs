    ...
    public partial class MapClient
    {
        ...
        public void SendCapacity()
        {
            if (this.Character.Online)
            {
                ...
                if (Configuration.Instance.Version >= SagaLib.Version.Saga18)
                {
                    p.Capacity = this.Character.Inventory.Volume[ContainerType.BACK_BAG] + this.Character.Inventory.Volume[ContainerType.BODY]+
                                                        this.Character.Inventory.Volume[ContainerType.LEFT_BAG]+this.Character.Inventory.Volume[ContainerType.RIGHT_BAG];
                    p.Payload = this.Character.Inventory.Payload[ContainerType.BACK_BAG] + this.Character.Inventory.Payload[ContainerType.BODY] +
                                                        this.Character.Inventory.Payload[ContainerType.LEFT_BAG] +this.Character.Inventory.Payload[ContainerType.RIGHT_BAG];
                    p.MaxCapacity = this.Character.Inventory.MaxVolume[ContainerType.BACK_BAG] +this.Character.Inventory.MaxVolume[ContainerType.BODY]+
                                                        this.Character.Inventory.MaxVolume[ContainerType.LEFT_BAG]+ this.Character.Inventory.MaxVolume[ContainerType.RIGHT_BAG];
                    p.MaxPayload = this.Character.Inventory.MaxPayload[ContainerType.BACK_BAG] + this.Character.Inventory.MaxPayload[ContainerType.BODY] +
                                    this.Character.Inventory.MaxPayload[ContainerType.LEFT_BAG] + this.Character.Inventory.MaxPayload[ContainerType.RIGHT_BAG];
                }
                else
                {
        ...
        public void SendGoldUpdate()
        {
            ...
            {
                if (Configuration.Instance.Version >= SagaLib.Version.Saga18)
                    this.Character.GoldLimit = 999999999999;
                else
                    this.Character.GoldLimit = 99999999;
        ...
        public void SendECoinUpdate()
        {
            if (this.Character.Online)
            {
                Packets.Server.SSMG_PLAYER_ECOIN_UPDATE p = new Packets.Server.SSMG_PLAYER_ECOIN_UPDATE();
        ...
        public void OnTitleSet(Packets.Client.CSMG_PLAYER_TITLE_SET p)
        {
            //TODO: Preresquite check and give ack
            bool NeedUpdateStatus=false;
            if (this.Character.MainTitle != p.Titles[3])
                NeedUpdateStatus = true;
            byte result = 0;
            if (result==0)
            {
                this.Character.FirstTitle = p.Titles[0];
                this.Character.SecondTitle = p.Titles[1];
                this.Character.ThirdTitle = p.Titles[2];
                this.Character.MainTitle = p.Titles[3];
                if (NeedUpdateStatus)
                {
                    PC.StatusFactory.Instance.CalcStatus(this.Character);
                }
            }
            this.Character.e.PropertyUpdate(UpdateEvent.Title, 0);
        }

        public void SendTitleUpdate()
        {
            Packets.Server.SSMG_PLAYER_TITLE_SET_RESULT p1 = new Packets.Server.SSMG_PLAYER_TITLE_SET_RESULT();
            p1.Result = 0;
            List<uint> titles = new List<uint>();
            titles.Add(Character.FirstTitle);
            titles.Add(Character.SecondTitle);
            titles.Add(Character.ThirdTitle);
            titles.Add(Character.MainTitle);
            p1.PutTitles(titles);
            this.netIO.SendPacket(p1);
        }

        public void OnTitleCheckProgress(Packets.Client.CSMG_PLAYER_TITLE_CHECK_PROGRESS p)
        {
            uint id = p.Title;
            if (SagaDB.Title.TitleFactory.Instance.Items.ContainsKey(id))
            {
                Packets.Server.SSMG_PLAYER_TITLE_PROGRESS p1 = new Packets.Server.SSMG_PLAYER_TITLE_PROGRESS();
                p1.Title = SagaDB.Title.TitleFactory.Instance.Items[id].ID;
                p1.Prerequisite = SagaDB.Title.TitleFactory.Instance.Items[id].Prerequisites.Values.ToList<ulong>();
                p1.PutPrerequisite();
                this.netIO.SendPacket(p1);
            }
        }
        ...
        public void SendTitleList()
        {
            ...
            titles.Add(Character.Titles[Page.One].Value);
            titles.Add(Character.Titles[Page.Two].Value);
            titles.Add(Character.Titles[Page.Three].Value);
            titles.Add(Character.Titles[Page.Four].Value);
            titles.Add(Character.Titles[Page.Five].Value);
            titles.Add(Character.Titles[Page.Six].Value);
            titles.Add(Character.Titles[Page.Seven].Value);
            titles.Add(Character.Titles[Page.Eight].Value);
            titles.Add(Character.Titles[Page.Nine].Value);
            titles.Add(Character.Titles[Page.Ten].Value);
            p.PutTitles(titles);
            List<long> newtitles = new List<long>();
            titles.Add(Character.NewTitles[Page.One].Value);
            titles.Add(Character.NewTitles[Page.Two].Value);
            titles.Add(Character.NewTitles[Page.Three].Value);
            titles.Add(Character.NewTitles[Page.Four].Value);
            titles.Add(Character.NewTitles[Page.Five].Value);
            titles.Add(Character.NewTitles[Page.Six].Value);
            titles.Add(Character.NewTitles[Page.Seven].Value);
            titles.Add(Character.NewTitles[Page.Eight].Value);
            titles.Add(Character.NewTitles[Page.Nine].Value);
            titles.Add(Character.NewTitles[Page.Ten].Value);
            p.PutNewTitles(newtitles);
            this.netIO.SendPacket(p);
        }

        public void OnTitleGet(Packets.Client.CSMG_PLAYER_TITLE_GET p)
        {
