    ...
    public partial class MapClient
    {
            ...
            if (this.currentShop != null)
            {
                ...
                switch (this.currentShop.ShopType)
                {
                    case ShopType.None:
                        ...
                        break;
                    case ShopType.CP:
                        this.Character.CP = (uint)gold;
                        break;
                    case ShopType.ECoin:
                        this.Character.ECoin = (uint)gold;
                        break;                
                }
                ...
                if (this.currentEvent != null)
                {
                    ...
                    for (int i = 0; i < goods.Length; i++)
                    {
                        if (this.currentEvent.Goods.Contains(goods[i]))
                        {
                            Item item = ItemFactory.Instance.GetItem(goods[i]);
                            item.Stack = (ushort)counts[i];
                            int price = (int)(item.BaseData.price * (((float)(100 + this.Character.Status.buy_rate)) / 100));
                            if (price == 0) price = 1;
                            price = price * item.Stack;
                            if (gold >= price)
                            {
        ...
        public void OnNPCShopSell(Packets.Client.CSMG_NPC_SHOP_SELL p)
        {
            ...
            if (this.currentShop != null)            
            {
                ...
                this.Character.Gold += (long)total;
                this.Character.Inventory.CalcPayloadVolume();
                this.SendCapacity();
            }
            else
        ...
        public void OnNPCSelect(Packets.Client.CSMG_NPC_SELECT p)
        {
            ...
            {
                Packets.Server.SSMG_NPC_SELECT_ACK p1 = new Packets.Server.SSMG_NPC_SELECT_ACK();
                p1.Result = 0;
                this.netIO.SendPacket(p1);
            }
        ...
        public void OnNPCSynthese(Packets.Client.CSMG_NPC_SYNTHESE p)
        {
            uint id = p.SynID;
            uint count = p.Count;
            if (!this.syntheseItem.ContainsKey(id))
                this.syntheseItem.Add(id, count);
        }
        ...
        public void OnNPCEventStart(Packets.Client.CSMG_NPC_EVENT_START p)
        {
            if (this.scriptThread == null)
            {
                ...
                //if (p.EventID < 20000000 || p.EventID >= 0xF0000000)Unknow为啥要限制编号 .  |   F子: 就是啊 好脑残
                //脑残啊還改成if true
        ...
        {
            if(this.Character.Account.GMLevel >= 200)
            this.SendSystemMessage("Event ID = " + EventID.ToString());
            ...
            else
            {
                ...
                {
                    SendNPCMessage(EventID, string.Format(LocalManager.Instance.Strings.NPC_EventID_NotFound_Msg, EventID), 131, "神经兮兮的人");
                }
                SendNPCMessageEnd();
                SendEventEnd();
        ...
        public void SendEventEnd()
        {
            ...
            if (Configuration.Instance.Version < SagaLib.Version.Saga18)
            {
                Packets.Server.SSMG_NPC_EVENT_ACK p = new Packets.Server.SSMG_NPC_EVENT_ACK();
                this.netIO.SendPacket(p);
            }
            Packets.Server.SSMG_NPC_EVENT_END p1 = new SagaMap.Packets.Server.SSMG_NPC_EVENT_END();
            this.netIO.SendPacket(p1);            
        }
        ...
        {
            if (SagaMap.Configuration.Instance.Version >= SagaLib.Version.Saga18)
            {
                if (this.Character.NPCStates.ContainsKey(this.map.ID))
                {
                    List<Dictionary<uint, bool>> states = new List<Dictionary<uint, bool>>();
                    int unloadedCount = this.chara.NPCStates[this.map.ID].Count;
                    int loadedCount = 0;
                    while (unloadedCount > 0)
                    {
                        if (unloadedCount > 100)
                        {
                            states.Add(this.chara.NPCStates[this.map.ID].Skip(loadedCount).Take(100).ToDictionary(i => i.Key, i => i.Value));
                            loadedCount += 100;
                            unloadedCount -= 100;
                        }
                        else
                        {
                            states.Add(this.chara.NPCStates[this.map.ID].Skip(loadedCount).Take(unloadedCount).ToDictionary(i => i.Key, i => i.Value));
                            loadedCount += unloadedCount;
                            unloadedCount = 0;
                        }
                    }
                    foreach (Dictionary<uint, bool> subpage in states)
                    {
