...
namespace SagaMap.Network.Client
{
    public partial class MapClient
    {
        ...
        public void OnGolemShopSellBuy(Packets.Client.CSMG_GOLEM_SHOP_SELL_BUY p)
        {
            Actor actor = this.map.GetActor(p.ActorID);
            Dictionary<uint, ushort> items = p.Items;
            int result = CheckBuyFromGolemShop(actor, items);
            if (result >=0)
            {
                ActorGolem golem = (ActorGolem)actor;
                uint gold = 0;
                foreach (uint i in items.Keys)
                {
                    ...
                    newItem.Stack = items[i];
                    if (newItem.Stack > 0)
                        Logger.LogItemLost(Logger.EventType.ItemGolemLost, this.Character.Name + "(" + this.Character.CharID + ")", newItem.BaseData.name
                            + "(" + newItem.ItemID + ")", string.Format("GolemSell Count:{0}", items[i]), false);
        ...
        int CheckBuyFromGolemShop(Actor actor, Dictionary<uint, ushort> items)
        {
            if (actor.type != ActorType.GOLEM)
                return -1; //なんらかの原因で失敗しました
            ActorGolem golem = (ActorGolem)actor;
            int itemInInventory = 0;
            uint gold=0;
            foreach (var itemlist in this.Character.Inventory.Items)
                itemInInventory += itemlist.Value.Count;
            foreach (uint i in items.Keys)
            {
                Item item = golem.Owner.Inventory.GetItem(i);
                if (item.BaseData.noTrade)
                    return -1; //なんらかの原因で失敗しました
                if (golem.SellShop.Count == 0)
                    return -2; //ゴーレムに出品されている商品が無くなりました
                if (this.Character == golem.Owner)
                    return -3; //自分のゴーレムからは買えません
                if (item == null)
                    return -4; //指定アイテムがありませんでした
                if (item.Stack <= items[i])
                    return -5; //指定アイテムが足りません
                gold += (golem.SellShop[i].Price * items[i]);
                if (this.Character.Gold < gold)
                    return -7; //所持金が足りません
                if (itemInInventory + item.Stack > 100)
                    return -8; //これ以上アイテムを所持することはできません
                if (gold + golem.Owner.Gold >= this.Character.GoldLimit)
                    return -9; //ゴーレムの所持金が上限に達したためキャンセルされました
            }
            return 0;
        }
        ...
        public void OnGolemShopOpen(Packets.Client.CSMG_GOLEM_SHOP_OPEN p)
        {
            ...
            if (actor.type == ActorType.GOLEM)
            {
                ...
                if (golem.GolemType == GolemType.Buy)
                {
                    ...
                    p3.BuyLimit = (uint)golem.BuyLimit;
                    this.netIO.SendPacket(p3);
                }
            }
        }
        ...
        public void OnGolemShopBuy(Packets.Client.CSMG_GOLEM_SHOP_BUY p)
        {
            ...
            p1.BuyLimit = (uint)this.Character.Golem.BuyLimit;
            p1.Comment = this.Character.Golem.Title;
