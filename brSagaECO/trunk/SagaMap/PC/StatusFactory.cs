...
namespace SagaMap.PC
{
    public partial class StatusFactory : Singleton<StatusFactory>
    {
        ...
        {
            //获取玩家基础能力
            ushort pcstr = (ushort)checkPositive((pc.Str + pc.Status.str_item + pc.Status.str_chip + pc.Status.str_rev + pc.Status.str_mario + pc.Status.str_skill + pc.Status.str_another));
            ushort pcdex = (ushort)checkPositive((pc.Dex + pc.Status.dex_item + pc.Status.dex_chip + pc.Status.dex_rev + pc.Status.dex_mario + pc.Status.dex_skill + pc.Status.dex_another));
            ushort pcint = (ushort)checkPositive((pc.Int + pc.Status.int_item + pc.Status.int_chip + pc.Status.int_rev + pc.Status.int_mario + pc.Status.int_skill + pc.Status.int_another));
            ushort pcvit = (ushort)checkPositive((pc.Vit + pc.Status.vit_item + pc.Status.vit_chip + pc.Status.vit_rev + pc.Status.vit_mario + pc.Status.vit_skill + pc.Status.vit_another));
            ushort pcagi = (ushort)checkPositive((pc.Agi + pc.Status.agi_item + pc.Status.agi_chip + pc.Status.agi_rev + pc.Status.agi_mario + pc.Status.agi_skill + pc.Status.agi_another));
            ushort pcmag = (ushort)checkPositive((pc.Mag + pc.Status.mag_item + pc.Status.mag_chip + pc.Status.mag_rev + pc.Status.mag_mario + pc.Status.mag_skill + pc.Status.mag_another));
            ...
            else
            {
                ...
                pc.Status.min_atk1 = (ushort)checkPositive((minatk + pc.Status.atk1_item + pc.Status.min_atk1_mario + pc.Status.min_atk1_skill + pc.Status.min_atk1_title + pc.Status.min_atk1_another + pc.Status.min_atk1_tamaire));
                pc.Status.min_atk2 = (ushort)checkPositive((minatk + pc.Status.atk2_item + pc.Status.min_atk2_mario + pc.Status.min_atk2_skill + pc.Status.min_atk2_title + pc.Status.min_atk2_another + pc.Status.min_atk2_tamaire));
                pc.Status.min_atk3 = (ushort)checkPositive((minatk + pc.Status.atk3_item + pc.Status.min_atk3_mario + pc.Status.min_atk3_skill + pc.Status.min_atk3_title + pc.Status.min_atk3_another + pc.Status.min_atk3_tamaire));
                ...
                pc.Status.max_atk1 = (ushort)checkPositive((maxatk + pc.Status.atk1_item + pc.Status.max_atk1_mario + pc.Status.max_atk1_skill + pc.Status.max_atk1_title + pc.Status.max_atk1_another + pc.Status.max_atk1_tamaire));
                pc.Status.max_atk2 = (ushort)checkPositive((maxatk + pc.Status.atk2_item + pc.Status.max_atk2_mario + pc.Status.max_atk2_skill + pc.Status.max_atk2_title + pc.Status.max_atk2_another + pc.Status.max_atk2_tamaire));
                pc.Status.max_atk3 = (ushort)checkPositive((maxatk + pc.Status.atk3_item + pc.Status.max_atk3_mario + pc.Status.max_atk3_skill + pc.Status.max_atk3_title + pc.Status.max_atk3_another + pc.Status.max_atk3_tamaire));
                ...
                pc.Status.min_matk = (ushort)checkPositive((minmatk + pc.Status.matk_item + pc.Status.min_matk_mario + pc.Status.min_matk_skill + pc.Status.min_matk_title + pc.Status.min_matk_another + pc.Status.min_matk_tamaire));
                ...
                pc.Status.max_matk = (ushort)checkPositive((maxmatk + pc.Status.matk_item + pc.Status.max_matk_mario + pc.Status.max_matk_skill + pc.Status.max_matk_title + pc.Status.max_matk_another + pc.Status.max_matk_tamaire));

                //命中计算
                ushort hit_melee = (ushort)(pcdex + (short)Math.Floor((double)((float)pcdex / 10.0f)) * 11 + pc.Level + 3);
                pc.Status.hit_melee = (ushort)checkPositive((hit_melee + pc.Status.hit_melee_item + pc.Status.hit_melee_skill + pc.Status.hit_melee_another + pc.Status.hit_melee_title + pc.Status.hit_melee_tamaire));

                ushort hit_ranged = (ushort)(pcint + (short)Math.Floor((double)((float)pcint / 10.0f)) * 11 + pc.Level + 3);
                pc.Status.hit_ranged = (ushort)checkPositive((hit_ranged + pc.Status.hit_ranged_item + pc.Status.hit_ranged_skill + pc.Status.hit_ranged_another + pc.Status.hit_ranged_title + pc.Status.hit_ranged_tamaire));
                ...
                pc.Status.def_add = (short)checkPositive((pc.Status.def_add + (pc.Status.def_add_mario + pc.Status.def_add_skill + pc.Status.def_add_another + pc.Status.def_add_title + pc.Status.def_add_tamaire)));
                ...
                pc.Status.mdef_add = (short)checkPositive((pc.Status.mdef_add + (pc.Status.mdef_add_mario + pc.Status.mdef_add_skill + pc.Status.mdef_add_another + pc.Status.mdef_add_title + pc.Status.mdef_add_tamaire)));

                //闪避计算
                ushort avoid_melee = (ushort)(pcagi + Math.Pow(Math.Floor((float)(pcagi + 18) / 9.0f), 2) + Math.Floor((float)pc.Level / 3.0f) - 1);
                pc.Status.avoid_melee = (ushort)checkPositive((avoid_melee + pc.Status.avoid_melee_item + pc.Status.avoid_melee_skill + pc.Status.avoid_melee_another + pc.Status.avoid_melee_title + pc.Status.avoid_melee_tamaire));

                ushort avoid_ranged = (ushort)(Math.Floor((float)pcint * 5.0f / 3.0f) + pcagi + Math.Floor((float)pc.Level / 3.0f) + 3);
                pc.Status.avoid_ranged = (ushort)checkPositive((avoid_ranged + pc.Status.avoid_ranged_item + pc.Status.avoid_ranged_skill + pc.Status.avoid_ranged_another + pc.Status.avoid_ranged_title + pc.Status.avoid_ranged_tamaire));

                //会心计算
                pc.Status.hit_critical = (ushort)(Math.Max(Math.Floor((double)((pcdex + 1) / 8)), 1));
                pc.Status.hit_critical = (ushort)checkPositive((pc.Status.hit_critical +  pc.Status.cri_title));
                pc.Status.avoid_critical = (ushort)((float)(pcagi + 8) / 6.0f);
                pc.Status.avoid_critical = (ushort)checkPositive((pc.Status.avoid_critical + pc.Status.cri_avoid_title));
        ...
        uint CalcMaxHP(ActorPC pc)
        {
            ...
            basehp = (uint)(basehp + pc.Status.hp_item + pc.Status.hp_mario + possession + pc.Status.hp_skill + pc.Status.hp_title + pc.Status.hp_another + pc.Status.hp_tamaire);
        ...
        uint CalcMaxMP(ActorPC pc)
        {
            ...
            uint basemp = (uint)Math.Floor((float)(pcmag * 3 + pc.Level + Math.Pow((float)Math.Floor((float)pc.Level / 9.0f), 2) + 30) * MPJobFactor(pc));
            basemp = (uint)(basemp + pc.Status.mp_item + pc.Status.mp_mario + possession + pc.Status.mp_skill + pc.Status.mp_title + pc.Status.mp_another + pc.Status.mp_tamaire);
        ...
        uint CalcMaxSP(ActorPC pc)
        {
            ...
            basesp = (uint)(basesp + pc.Status.sp_item + pc.Status.sp_mario + possession + pc.Status.sp_skill + pc.Status.sp_title + pc.Status.sp_another + pc.Status.sp_tamaire);
        ...
        void CalcStatsRev(ActorPC pc)
        {
            if (pc.JobJoint == PC_JOB.NONE)
            {
                byte joblv1 = pc.JobLevel1;
                byte joblv2x = pc.JobLevel2X;
                byte joblv2t = pc.JobLevel2T;
                byte joblv3 = pc.JobLevel3;
