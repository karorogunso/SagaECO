...
namespace SagaMap.Skill.SkillDefinations.Cardinal
{
    class Frustrate : ISkill
    {
        ...
        void StartEventHandler(Actor actor, DefaultBuff skill)
        {
            ...
            skill.Variable.Add("Frustrate_DEF", (int)(actor.Status.def * rank[skill.skill.Level]));
            actor.Status.def_skill -= (short)(actor.Status.def * rank[skill.skill.Level]);

            if (skill.Variable.ContainsKey("Frustrate_MDEF"))
                skill.Variable.Remove("Frustrate_MDEF");
            skill.Variable.Add("Frustrate_MDEF", (int)(actor.Status.mdef * rank[skill.skill.Level]));
            actor.Status.mdef_skill -= (short)(actor.Status.mdef * rank[skill.skill.Level]);
