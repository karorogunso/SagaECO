...
namespace SagaMap.Mob.AICommands
{
    public class Attack : AICommand
    {
            ...
            try
            {
                ...
                for (uint i = 0; i < mob.Hate.Keys.Count; i++)//Find out the actorPC with the highest hate value
                {
                    ...
                    if (hate < mob.Hate[ids[i]])
                    {
                        ...
                        if (tmp.Status.Additions.ContainsKey("Invisible"))
                        {
                            mob.Hate.Remove(id);
                            continue;
                        }
        ...
        public void Update(object para)
        {
            ...
            {
                if (DateTime.Now > this.mob.BackTimer.AddSeconds(15))
                    this.mob.BackTimer = DateTime.Now;
                if (DateTime.Now.AddSeconds(20) > this.mob.BackTimer.AddSeconds(30) && attacking)//5秒后返回
                {
            ...
            else
            {
                if (!this.mob.Mode.RunAway || Global.Random.Next(0, 99) < 70)
                {
                    if (mob.CanAttack)
                    {
                        ...
                        if (!attacktask.Activated) attacktask.Activate();
                        attacking = true;
                    }
                }
                else
