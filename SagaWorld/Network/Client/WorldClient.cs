...
using SagaLib;
using SagaWorld;
using SagaWorld.Manager;

namespace SagaWorld.Network.Client
{
    public class WorldClient : SagaLib.Client
    {
        ...
        public WorldClient(Socket mSock, Dictionary<ushort, Packet> mCommandTable)
        {
            this.netIO = new NetIO(mSock, mCommandTable, this, WorldClientManager.Instance);
        ...
        public void OnSendVersion(Packets.Client.CSMG_SEND_VERSION p)
        {
            ...
            Packets.Server.SSMG_VERSION_ACK p1 = new SagaWorld.Packets.Server.SSMG_VERSION_ACK();
            p1.SetResult(SagaWorld.Packets.Server.SSMG_VERSION_ACK.Result.OK);
            ...
            Packets.Server.SSMG_LOGIN_ALLOWED p2 = new SagaWorld.Packets.Server.SSMG_LOGIN_ALLOWED();
        ...
        {            
            Packets.Server.SSMG_LOGIN_ALLOWED p1 = new SagaWorld.Packets.Server.SSMG_LOGIN_ALLOWED();
            this.netIO.SendPacket(p1);
        }

        public void OnPing(Packets.Client.CSMG_PING p)
        {
            Packets.Server.SSMG_PONG p1 = new SagaWorld.Packets.Server.SSMG_PONG();
            this.netIO.SendPacket(p1);
        }
        ...
        public void OnLogin(Packets.Client.CSMG_LOGIN p)
        {
            ...
            {
                Packets.Server.SSMG_LOGIN_ACK p1 = new SagaWorld.Packets.Server.SSMG_LOGIN_ACK();
                p1.LoginResult = SagaWorld.Packets.Server.SSMG_LOGIN_ACK.Result.GAME_SMSG_LOGIN_ERR_BADPASS;
                this.netIO.SendPacket(p1);                
            }
        ...
        public void OnCharCreate(Packets.Client.CSMG_CHAR_CREATE p)
        {
            ...
            else
            {
                ...
                if (slot.Count() != 0)
                {
                    p1.CreateResult = SagaWorld.Packets.Server.SSMG_CHAR_CREATE_ACK.Result.GAME_SMSG_CHRCREATE_E_ALREADY_SLOT;
                }
                else
                {
        ...
        {
            Packets.Server.SSMG_CHAR_DELETE_ACK p1 = new SagaWorld.Packets.Server.SSMG_CHAR_DELETE_ACK();
            var chr =
            ...
            {
                p1.DeleteResult = SagaWorld.Packets.Server.SSMG_CHAR_DELETE_ACK.Result.WRONG_DELETE_PASSWORD;
            }
            this.netIO.SendPacket(p1);
            this.SendCharData();
        }
        ...
        {
            Packets.Server.SSMG_CHAR_SELECT_ACK p1 = new SagaWorld.Packets.Server.SSMG_CHAR_SELECT_ACK();
            var chr =
                from c in account.Characters
                where c.Slot == p.Slot
                select c;
        ...
        {
            Packets.Server.SSMG_SEND_TO_MAP_SERVER p1 = new SagaWorld.Packets.Server.SSMG_SEND_TO_MAP_SERVER();
            p1.ServerID = 1;
        ...
        {
            Packets.Server.SSMG_CHAR_STATUS p1 = new SagaWorld.Packets.Server.SSMG_CHAR_STATUS();
            this.netIO.SendPacket(p1);
        }

        private void SendCharData()
        {
            Packets.Server.SSMG_CHAR_DATA p2 = new SagaWorld.Packets.Server.SSMG_CHAR_DATA();
            p2.Chars = account.Characters;
            this.netIO.SendPacket(p2);
            Packets.Server.SSMG_CHAR_EQUIP p3 = new SagaWorld.Packets.Server.SSMG_CHAR_EQUIP();
